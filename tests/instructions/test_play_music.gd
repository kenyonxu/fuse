...
func _ready():
	print("=== Testing Play Music Instruction ===")

	test_music_path_validation()
	test_volume_validation()
	test_bus_validation()
	test_fade_in_validation()
	test_invalid_music_path()

	print("=== All Play Music tests passed! ===")

## 测试 1: 音乐路径验证
func test_music_path_validation():
	print("Test 1: Music path validation")

	var instruction_script = load("res://addons/fuse/instructions/play_music.gd")
...
## 测试 4: 淡入验证
func test_fade_in_validation():
	print("Test 4: Fade in validation")
	...
	# 测试无效的淡入时间
	instruction.fade_in = true
	instruction.fade_duration = -0.1
	var errors = instruction.validate()
	assert(errors.size() > 0, "Should have errors for negative fade duration")
	print("  Negative fade duration validation: ✓")

	# 测试有效的淡入时间
	instruction.fade_duration = 2.0
	errors = instruction.validate()
	assert(not "淡入时间必须大于 0" in errors, "Should not have fade duration error")
	print("  Valid fade duration validation: ✓")
	print("  ✓ Test 4 passed\n")
