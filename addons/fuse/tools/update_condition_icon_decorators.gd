...
func _run():
	...
	# 扫描所有条件文件
	var condition_files = _scan_condition_files(CONDITIONS_DIR)
	print("找到 %d 个条件文件\n" % condition_files.size())
	...
	for file_path in condition_files:
		var result = _update_condition_file(file_path, debug_mode)
		match result:
			0: # 已跳过
				skipped_count += 1
			1: # 已更新
...
## 递归扫描条件目录
func _scan_condition_files(dir_path: String) -> Array[String]:
	...
	while file_name != "":
		...
		if dir.current_is_dir():
			# 递归扫描子目录
			files.append_array(_scan_condition_files(full_path))
...
## 更新单个条件文件
## 返回: 1 = 已更新, 0 = 已跳过, -1 = 错误
func _update_condition_file(file_path: String, debug_mode: bool = false) -> int:
	# 读取文件内容
	...
	# 检查是否是条件文件（通过检查是否 extends BaseCondition）
	var is_condition = "extends BaseCondition" in content

	if not is_condition:
		if debug_mode:
			print("  [DEBUG] 跳过 %s (不是条件类)" % file_path.get_file())
		return 0
