...
func _run():
	...
	# 扫描所有指令文件
	var instruction_files = _scan_instruction_files(INSTRUCTIONS_DIR)
	print("找到 %d 个指令文件\n" % instruction_files.size())
	...
	for file_path in instruction_files:
		var result = _update_instruction_file(file_path, debug_mode)
		match result:
			0: # 已跳过
				skipped_count += 1
			1: # 已更新
...
## 递归扫描指令目录
func _scan_instruction_files(dir_path: String) -> Array[String]:
	...
	while file_name != "":
		...
		if dir.current_is_dir():
			# 递归扫描子目录
			files.append_array(_scan_instruction_files(full_path))
...
## 更新单个指令文件
## 返回: 1 = 已更新, 0 = 已跳过, -1 = 错误
func _update_instruction_file(file_path: String, debug_mode: bool = false) -> int:
	# 读取文件内容
	...
	# 检查是否是指令文件（通过检查是否包含 instruction 相关的 extends）
	var is_instruction = (
		"extends BaseInstruction" in content or
		"extends BaseEvent" in content or
		"extends BaseCondition" in content or
		"extends BaseTweenInstruction" in content or
		"extends BasePropertyInstruction" in content
	)

	if not is_instruction:
		if debug_mode:
			print("  [DEBUG] 跳过 %s (不是指令/事件/条件类)" % file_path.get_file())
		return 0
