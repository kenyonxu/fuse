...
func _ready():
	...
	test_validation()
	print("=== All OnArea2DExited tests passed! ===")
...
func test_basic_functionality():
	...
	body_shape_rect.size = Vector2(50, 50)
	body_shape.shape = body_shape_rect
	body.add_child(body_shape)
	add_child(body)

	# 创建事件
	var event = OnArea2DExited.new()
	...
	# 将物体移出区域
	body.position = area.position + Vector2(200, 200)
	await get_tree().create_timer(0.1).timeout
...
func test_target_group_filtering():
	...
	var player_shape_rect = RectangleShape2D.new()
	player_shape_rect.size = Vector2(50, 50)
	player_shape.shape = player_shape_rect
	...
	var enemy_shape_rect = RectangleShape2D.new()
	enemy_shape_rect.size = Vector2(50, 50)
	enemy_shape.shape = enemy_shape_rect
	...
	# 移出 player
	player_body.position = area.position + Vector2(200, 200)
	await get_tree().create_timer(0.1).timeout
...
func test_area_exited_signal():
	...
	# 将 area2 移出 area1
	area2.position = area1.position + Vector2(200, 200)
	await get_tree().create_timer(0.1).timeout
