...
func test_variable_comparison_condition():
	...
	assert(result == true, "50 > 42 应该为 true")
...
func test_node_exists_condition():
	...
	assert(deps.is_empty(), "节点检查不应该有变量依赖")
...
func test_if_else_with_condition():
	...
	assert("If/Else" in desc, "描述应包含 If/Else")
...
func test_condition_cache():
	...
	assert(check_count_after > check_count_before, "第一次检查应该增加计数")
