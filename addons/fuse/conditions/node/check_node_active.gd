@tool
@icon("res://addons/fuse/icons/builtin/GuiVisibilityVisible.png")
...
## 检查类型
var check_type: CheckType = CheckType.VISIBLE:
	set(value):
		check_type = value
		_update_resource_name()
...
func _get_property_list() -> Array[Dictionary]:
	...
	properties.append({
		...
		type = TYPE_INT,
		hint = PROPERTY_HINT_ENUM,
		hint_string = "Visible,Processing,Inside Tree",
		usage = PROPERTY_USAGE_DEFAULT | PROPERTY_USAGE_SCRIPT_VARIABLE
	})
...
func _validate_property(property: Dictionary) -> void:
	...
	if node_source == NodeSource.NODE_PATH:
		...
		if property.name == "check_node_path":
			property.usage = PROPERTY_USAGE_NO_EDITOR
...
func _get_node_source_string() -> String:
	if node_source == NodeSource.NODE_PATH:
		var path_str = _get_node_display_name(check_node_path)
...
func _get_check_type_name() -> String:
	match check_type:
		CheckType.VISIBLE: return FuseLocalization.translate("FUSE_CONDITION_NODE_ACTIVE_TYPE_VISIBLE")
		CheckType.PROCESSING: return FuseLocalization.translate("FUSE_CONDITION_NODE_ACTIVE_TYPE_PROCESSING")
		CheckType.INSIDE_TREE: return FuseLocalization.translate("FUSE_CONDITION_NODE_ACTIVE_TYPE_INSIDE_TREE")
		_: return FuseLocalization.translate("FUSE_CONDITION_NODE_ACTIVE_TYPE_UNKNOWN")
...
func _evaluate_condition(context: ExecutionContext) -> bool:
	...
	match check_type:
		...
		_:
			var error_msg = FuseLocalization.translate("FUSE_ERROR_UNKNOWN_CHECK_TYPE") % check_type
			_log_error(error_msg)
			return false
...
func _get_node_by_path(context: ExecutionContext) -> Node:
	...
	if check_node_path.is_empty():
		var error_msg = FuseLocalization.translate("FUSE_ERROR_CHECK_NODE_PATH_EMPTY")
		_log_error(error_msg)
		_create_fuse_error(error_msg, FuseError.ErrorType.VALIDATION_ERROR)
		return null
...
## 声明变量读写模式（精确化静态分析）
## node_variable_name 仅 read（_evaluate_condition 中通过变量解析节点路径并读取激活状态）
...
func get_parameters() -> Dictionary:
	...
	if node_source == NodeSource.NODE_PATH:
		params["check_node_path"] = check_node_path
	else:
...
static func _get_condition_metadata() -> ConditionMetadata:
	...
	metadata.name_key = "FUSE_CONDITION_NODE_ACTIVE_NAME"
	metadata.category_key = "FUSE_CATEGORY_NODE_OPERATIONS"
	metadata.description_key = "FUSE_CONDITION_NODE_ACTIVE_DESC"
	metadata.keywords = ["节点", "激活", "可见", "visible", "active", "enable", "show", "变量", "variable"]
