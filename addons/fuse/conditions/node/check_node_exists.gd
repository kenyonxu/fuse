...
func _update_resource_name() -> void:
	...
	if node_str.is_empty():
		resource_name = FuseLocalization.translate("FUSE_CONDITION_NODE_EXISTS_NOT_SET")
	else:
		var translated = FuseLocalization.translate("FUSE_CONDITION_NODE_EXISTS_FORMAT")
		resource_name = translated.format({"path": node_str})
...
## 声明变量读写模式（精确化静态分析）
## node_variable_name 仅 read（_evaluate_condition 中通过变量解析节点路径判断存在性）
...
func get_description() -> String:
	...
	if node_str.is_empty():
		return FuseLocalization.translate("FUSE_CONDITION_NODE_EXISTS_DESC_NOT_SET")

	var desc_base = FuseLocalization.translate("FUSE_CONDITION_NODE_EXISTS_FORMAT")
	var desc = desc_base.format({"path": node_str})
...
static func _get_condition_metadata() -> ConditionMetadata:
	...
	metadata.name_key = "FUSE_CONDITION_NODE_EXISTS_NAME"
	metadata.category_key = "FUSE_CATEGORY_NODE_OPERATIONS"
	metadata.description_key = "FUSE_CONDITION_NODE_EXISTS_DESC"
	metadata.keywords = ["节点", "存在", "检查", "node", "exists", "check", "find", "isvalid", "valid", "变量", "variable"]
