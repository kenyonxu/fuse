@tool
@icon("res://addons/fuse/icons/builtin/Groups.svg")
extends BaseCondition
class_name CheckNodeInGroup

## 节点组检测条件
##
## 检查节点是否属于指定的组。适用于批量对象管理和分类。
## 使用 Godot 原生的 is_in_group() 方法，性能优异。
##
## 支持两种节点来源：
...
func _update_resource_name() -> void:
	...
	if node_str.is_empty():
		resource_name = FuseLocalization.translate("FUSE_CONDITION_NODE_GROUP_NOT_SET")
	elif group_name.is_empty():
		resource_name = FuseLocalization.translate_format("FUSE_CONDITION_NODE_GROUP_NO_GROUP", {"path": node_str})
	else:
		resource_name = FuseLocalization.translate_format("FUSE_CONDITION_NODE_GROUP_FORMAT", {"path": node_str, "group": group_name})
...
func _evaluate_condition(context: ExecutionContext) -> bool:
	...
	# 检查节点是否在组中
	var in_group = node.is_in_group(group_name)

	var result_str = FuseLocalization.translate("FUSE_LOG_YES" if in_group else "FUSE_LOG_NO")
	var log_msg = FuseLocalization.translate_format("FUSE_LOG_NODE_GROUP_CHECK", {
		"path": _get_node_source_string(),
		"group": group_name,
		"result": result_str
	})
	_log_debug(log_msg)
...
func _get_node_by_variable(context: ExecutionContext) -> Node:
	...
	# 验证变量值
	if node_value == null:
		var error_msg = FuseLocalization.translate_format("FUSE_ERROR_NODE_VARIABLE_IS_NULL", {"name": node_variable_name})
		_log_error(error_msg)
		_create_fuse_error(error_msg, FuseError.ErrorType.RUNTIME_ERROR)
		return null
	...
	if node_value is Node:
		...
		if node == null:
			var error_msg = FuseLocalization.translate("FUSE_ERROR_NODE_NOT_FOUND") % str(node_value)
			_log_error(error_msg)
			_create_fuse_error(error_msg, FuseError.ErrorType.RUNTIME_ERROR)
			return null
		...
		if node == null:
			var error_msg = FuseLocalization.translate("FUSE_ERROR_NODE_NOT_FOUND") % node_value
			_log_error(error_msg)
			_create_fuse_error(error_msg, FuseError.ErrorType.RUNTIME_ERROR)
			return null
	else:
...
## 声明变量读写模式（精确化静态分析）
## node_variable_name 仅 read（_evaluate_condition 中通过变量解析节点路径并检查组归属）
...
func get_description() -> String:
	...
	if node_str.is_empty():
		return FuseLocalization.translate("FUSE_CONDITION_NODE_GROUP_NOT_SET")

	if group_name.is_empty():
		return FuseLocalization.translate_format("FUSE_CONDITION_NODE_GROUP_NO_GROUP", {"path": node_str})

	var desc = FuseLocalization.translate_format("FUSE_CONDITION_NODE_GROUP_FORMAT_SHORT", {"path": node_str, "group": group_name})
...
static func _get_condition_metadata() -> ConditionMetadata:
	...
	metadata.name_key = "FUSE_CONDITION_NODE_IN_GROUP_NAME"
	metadata.category_key = "FUSE_CATEGORY_NODE_OPERATIONS"
	metadata.description_key = "FUSE_CONDITION_NODE_IN_GROUP_DESC"
	metadata.keywords = ["组", "group", "节点组", "node group", "分类", "category", "批量", "batch", "标签", "tag", "is_in_group", "变量", "variable"]
	metadata.builtin_icon = "Groups"
	return metadata
