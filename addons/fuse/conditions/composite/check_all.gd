@tool
@icon("res://addons/fuse/icons/builtin/AnimationTrackList.png")
...
func _update_resource_name() -> void:
	if conditions.is_empty():
		resource_name = FuseLocalization.translate("FUSE_CONDITION_ALL_NO_CONDITIONS")
	else:
		var count = conditions.size()
		resource_name = FuseLocalization.translate_format("FUSE_CONDITION_ALL_WITH_COUNT", {"count": count})

## 评估条件
func _evaluate_condition(context: ExecutionContext) -> bool:
	# 验证条件列表
	if conditions.is_empty():
		_log_warning(FuseLocalization.translate("FUSE_CONDITION_WARNING_ALL_EMPTY"))
		return false
	...
	for i in range(conditions.size()):
		...
		# 短路求值:遇到 false 立即返回
		if not result:
			_log_debug(FuseLocalization.translate_format("FUSE_CONDITION_LOG_ALL_FAILED", {"index": i}))
			return false

	_log_debug(FuseLocalization.translate_format("FUSE_CONDITION_LOG_ALL_SUCCESS", {"count": conditions.size()}))
	return true
...
func get_description() -> String:
	if conditions.is_empty():
		return FuseLocalization.translate("FUSE_CONDITION_ALL_NO_CONDITIONS")

	var desc = FuseLocalization.translate_format("FUSE_CONDITION_ALL_WITH_COUNT", {"count": conditions.size()})
...
func validate() -> Array[String]:
	...
	if conditions.is_empty():
		errors.append(FuseLocalization.translate("FUSE_CONDITION_ERROR_ALL_NEEDS_ONE"))
	else:
		# 验证所有子条件
		for i in range(conditions.size()):
			var condition = conditions[i]
