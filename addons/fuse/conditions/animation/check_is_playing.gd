...
func _update_resource_name() -> void:
	...
	if node_str.is_empty():
		resource_name = FuseLocalization.translate("FUSE_CONDITION_IS_PLAYING_NOT_SET")
	else:
		resource_name = FuseLocalization.translate_format("FUSE_CONDITION_IS_PLAYING_WITH_PATH", {"path": node_str})
		# 添加变量来源信息
...
func _evaluate_condition(context: ExecutionContext) -> bool:
	...
	_log_debug(FuseLocalization.translate_format(
		"FUSE_CONDITION_LOG_IS_PLAYING_CHECK",
		{"node": _get_node_source_string(), "result": FuseLocalization.translate("FUSE_CONDITION_PLAYING" if is_playing else "FUSE_CONDITION_NOT_PLAYING")}
	))
...
## 声明变量读写模式（精确化静态分析）
## node_variable_name 仅 read（_evaluate_condition 中通过变量解析节点路径并检查播放状态）
...
func get_description() -> String:
	...
	if node_str.is_empty():
		return FuseLocalization.translate("FUSE_CONDITION_IS_PLAYING_DESC_NO_PATH")

	var desc = FuseLocalization.translate_format("FUSE_CONDITION_IS_PLAYING_WITH_PATH", {"path": node_str})
...
static func _get_condition_metadata() -> ConditionMetadata:
	...
	metadata.keywords = ["动画", "animation", "播放", "playing", "AnimationPlayer", "is_playing", "变量", "variable"]
