...
func _update_resource_name() -> void:
	...
	if node_str.is_empty():
		resource_name = FuseLocalization.translate("FUSE_CONDITION_ANIMATION_FINISHED_NO_NODE")
	elif not animation_name.is_empty():
		resource_name = FuseLocalization.translate_format("FUSE_CONDITION_ANIMATION_FINISHED_WITH_NAME", {"name": animation_name})
		# 添加变量来源信息
		if node_source == NodeSource.VARIABLE:
			resource_name += " [%s]" % node_str
	else:
		resource_name = FuseLocalization.translate("FUSE_CONDITION_ANIMATION_FINISHED_ANY")
...
func _evaluate_condition(context: ExecutionContext) -> bool:
	...
	if not animation_name.is_empty():
		...
		_log_debug(FuseLocalization.translate_format(
			"FUSE_CONDITION_LOG_ANIMATION_FINISHED_CHECK",
			{"name": animation_name, "result": FuseLocalization.translate("FUSE_CONDITION_FINISHED" if is_finished else "FUSE_CONDITION_NOT_FINISHED")}
		))
	else:
		# 未指定动画，检查是否任何动画都在播放
		is_finished = not node.is_playing()
		...
		_log_debug(FuseLocalization.translate_format(
			"FUSE_CONDITION_LOG_ANIMATION_FINISHED_CHECK",
			{"name": FuseLocalization.translate("FUSE_CONDITION_ANY_ANIMATION"), "result": FuseLocalization.translate("FUSE_CONDITION_FINISHED" if is_finished else "FUSE_CONDITION_NOT_FINISHED")}
		))
...
func get_description() -> String:
	...
	if node_str.is_empty():
		return FuseLocalization.translate("FUSE_CONDITION_ANIMATION_FINISHED_DESC_NO_NODE")

	if not animation_name.is_empty():
		var desc = FuseLocalization.translate_format("FUSE_CONDITION_ANIMATION_FINISHED_WITH_NAME", {"name": animation_name})
		if desc.length() > 50:
			desc = desc.substr(0, 47) + "..."
		return desc
	else:
		return FuseLocalization.translate("FUSE_CONDITION_ANIMATION_FINISHED_ANY")
...
static func _get_condition_metadata() -> ConditionMetadata:
	...
	metadata.keywords = ["动画", "animation", "完成", "finished", "end", "AnimationPlayer", "变量", "variable"]
