...
func _update_resource_name() -> void:
	...
	if node_str.is_empty():
		resource_name = FuseLocalization.translate("FUSE_CONDITION_IS_ANIMATION_NO_NODE")
	elif animation_name.is_empty():
		resource_name = FuseLocalization.translate("FUSE_CONDITION_IS_ANIMATION_NO_NAME")
	else:
		resource_name = FuseLocalization.translate_format("FUSE_CONDITION_IS_ANIMATION_WITH_NAME", {"name": animation_name})
		# 添加变量来源信息
...
func _evaluate_condition(context: ExecutionContext) -> bool:
	...
	if animation_name.is_empty():
		var error_msg = FuseLocalization.translate("FUSE_CONDITION_ERROR_ANIMATION_NAME_EMPTY")
	...
	_log_debug(FuseLocalization.translate_format(
		"FUSE_CONDITION_LOG_IS_ANIMATION_CHECK",
		{"expected": animation_name, "current": current_animation if current_animation else FuseLocalization.translate("FUSE_CONDITION_NONE"), "result": FuseLocalization.translate("FUSE_CONDITION_MATCH" if is_match else "FUSE_CONDITION_NOT_MATCH")}
	))
...
## 声明变量读写模式（精确化静态分析）
## node_variable_name 仅 read（_evaluate_condition 中通过变量解析节点路径并读取当前动画名）
...
func get_description() -> String:
	...
	if node_str.is_empty():
		return FuseLocalization.translate("FUSE_CONDITION_IS_ANIMATION_DESC_NO_NODE")
	if animation_name.is_empty():
		return FuseLocalization.translate("FUSE_CONDITION_IS_ANIMATION_NO_NAME")

	var desc = FuseLocalization.translate_format("FUSE_CONDITION_IS_ANIMATION_WITH_NAME", {"name": animation_name})
...
func validate() -> Array[String]:
	...
	if animation_name.is_empty():
		errors.append(FuseLocalization.translate("FUSE_CONDITION_ERROR_ANIMATION_NAME_EMPTY"))
...
static func _get_condition_metadata() -> ConditionMetadata:
	...
	metadata.keywords = ["动画", "animation", "名称", "name", "AnimationPlayer", "current_animation", "变量", "variable"]
