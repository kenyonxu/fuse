@tool
@icon("res://addons/fuse/icons/builtin/ArrowUp.png")
extends BaseCondition
class_name CheckInAir

## 在空中条件
##
## 检查 CharacterBody2D/3D 节点是否在空中。用于跳跃、下落等物理状态检测。
##
## 支持两种节点来源：
...
func _update_resource_name() -> void:
	...
	if node_str.is_empty():
		resource_name = FuseLocalization.translate("FUSE_CONDITION_IN_AIR_NOT_SET")
	else:
		var path_str = node_str
		# 限制长度
		if path_str.length() > 35:
			path_str = path_str.substr(0, 32) + "..."
		resource_name = FuseLocalization.translate_format("FUSE_CONDITION_IN_AIR_FORMAT", {"path": path_str})
...
func _evaluate_condition(context: ExecutionContext) -> bool:
	...
	if node is CharacterBody2D:
		in_air = not node.is_on_floor()
	elif node is CharacterBody3D:
		in_air = not node.is_on_floor()

	var result_text = "在空中" if in_air else "不在空中"
	var debug_msg = FuseLocalization.translate_format("FUSE_LOG_IN_AIR_CHECK", {"path": _get_node_source_string(), "result": result_text})
...
func get_description() -> String:
	...
	if node_str.is_empty():
		return FuseLocalization.translate("FUSE_CONDITION_IN_AIR_DESC_NOT_SET")

	var desc = FuseLocalization.translate_format("FUSE_CONDITION_IN_AIR_FORMAT", {"path": node_str})
...
static func _get_condition_metadata() -> ConditionMetadata:
	...
	metadata.name_key = "FUSE_CONDITION_IN_AIR_NAME"
	metadata.category_key = "FUSE_CATEGORY_PHYSICS"
	metadata.description_key = "FUSE_CONDITION_IN_AIR_DESC"
	metadata.keywords = ["空中", "air", "jump", "falling", "跳跃", "下落", "not_on_floor", "腾空", "变量", "variable"]
	metadata.builtin_icon = "ArrowUp"
	return metadata
