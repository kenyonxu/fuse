@tool
@icon("res://addons/fuse/icons/builtin/AnimationAutoFit.png")
extends BaseCondition
class_name CheckOnWall

## 在墙壁上条件
##
## 检查 CharacterBody2D/3D 节点是否在墙壁上。
##
## 支持两种节点来源：
...
func _update_resource_name() -> void:
	...
	if node_str.is_empty():
		resource_name = FuseLocalization.translate("FUSE_CONDITION_ON_WALL_NOT_SET")
	else:
		var path_str = node_str
		# 限制长度
		if path_str.length() > 35:
			path_str = path_str.substr(0, 32) + "..."
		resource_name = FuseLocalization.translate_format("FUSE_CONDITION_ON_WALL_FORMAT", {"path": path_str})
...
func _evaluate_condition(context: ExecutionContext) -> bool:
	...
	if node is CharacterBody2D:
		on_wall = node.is_on_wall()
	elif node is CharacterBody3D:
		on_wall = node.is_on_wall()

	var result_text = "在墙上" if on_wall else "未在墙上"
	var debug_msg = FuseLocalization.translate_format("FUSE_LOG_ON_WALL_CHECK", {"path": _get_node_source_string(), "result": result_text})
...
## 声明变量读写模式（精确化静态分析）
## node_variable_name 仅 read（_evaluate_condition 中通过变量解析 CharacterBody 节点并检查 is_on_wall）
...
func get_description() -> String:
	...
	if node_str.is_empty():
		return FuseLocalization.translate("FUSE_CONDITION_ON_WALL_DESC_NOT_SET")

	var desc = FuseLocalization.translate_format("FUSE_CONDITION_ON_WALL_FORMAT", {"path": node_str})
...
static func _get_condition_metadata() -> ConditionMetadata:
	...
	metadata.name_key = "FUSE_CONDITION_ON_WALL_NAME"
	metadata.category_key = "FUSE_CATEGORY_PHYSICS"
	metadata.description_key = "FUSE_CONDITION_ON_WALL_DESC"
	metadata.keywords = ["墙壁", "wall", "碰撞", "collision", "CharacterBody", "is_on_wall", "变量", "variable"]
	metadata.builtin_icon = "AnimationAutoFit"
	return metadata
