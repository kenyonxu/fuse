...
## 在地面上条件
##
## 检查 CharacterBody2D/3D 节点是否在地面上。用于跳跃、移动等物理状态检测。
##
## 支持两种节点来源：
...
func _update_resource_name() -> void:
	...
	if node_str.is_empty():
		resource_name = FuseLocalization.translate("FUSE_CONDITION_ON_FLOOR_NOT_SET")
	else:
		var path_str = node_str
		# 限制长度
		if path_str.length() > 35:
			path_str = path_str.substr(0, 32) + "..."
		resource_name = FuseLocalization.translate_format("FUSE_CONDITION_ON_FLOOR_FORMAT", {"path": path_str})
...
func _evaluate_condition(context: ExecutionContext) -> bool:
	...
	var result_text = "在地面" if on_floor else "不在地面"
	var debug_msg = FuseLocalization.translate_format("FUSE_LOG_ON_FLOOR_CHECK", {"path": _get_node_source_string(), "result": result_text})
...
func get_description() -> String:
	...
	if node_str.is_empty():
		return FuseLocalization.translate("FUSE_CONDITION_ON_FLOOR_DESC_NOT_SET")

	var desc = FuseLocalization.translate_format("FUSE_CONDITION_ON_FLOOR_FORMAT", {"path": node_str})
...
static func _get_condition_metadata() -> ConditionMetadata:
	...
	metadata.name_key = "FUSE_CONDITION_ON_FLOOR_NAME"
	metadata.category_key = "FUSE_CATEGORY_PHYSICS"
	metadata.description_key = "FUSE_CONDITION_ON_FLOOR_DESC"
	metadata.keywords = ["地面", "floor", "ground", "character", "碰撞", "collision", "is_on_floor", "着地", "变量", "variable"]
