...
## 速度阈值
var velocity_threshold: float = 0.0:
	set(value):
		velocity_threshold = value
		_update_resource_name()

## 比较运算符
var comparison_operator: int = 0:
	set(value):
...
func _get_property_list() -> Array[Dictionary]:
	...
	properties.append({
		...
		type = TYPE_INT,
		hint = PROPERTY_HINT_ENUM,
		hint_string = "Greater Than,Less Than,Equal,Greater Equal,Less Equal",
		usage = PROPERTY_USAGE_DEFAULT | PROPERTY_USAGE_SCRIPT_VARIABLE
	})
...
func _update_resource_name() -> void:
	...
	if node_str.is_empty():
		resource_name = FuseLocalization.translate("FUSE_CONDITION_VELOCITY_NOT_SET")
	else:
		var op_symbol = _get_operator_symbol()
		resource_name = FuseLocalization.translate_format("FUSE_CONDITION_VELOCITY_FORMAT", {"op": op_symbol, "value": velocity_threshold})
		# 添加节点来源信息
		if node_source == NodeSource.VARIABLE:
			resource_name += " [%s]" % node_str
...
func _evaluate_condition(context: ExecutionContext) -> bool:
	...
	if velocity is Vector2:
		...
		var error_msg = FuseLocalization.translate("FUSE_ERROR_VELOCITY_VECTOR_REQUIRED")
		_log_error(error_msg)
		_create_fuse_error(error_msg, FuseError.ErrorType.VALIDATION_ERROR)
		return false
	...
	var result = false
	match comparison_operator:
		0: result = velocity_magnitude > velocity_threshold  # 大于
		1: result = velocity_magnitude < velocity_threshold  # 小于
		2: result = is_equal_approx(velocity_magnitude, velocity_threshold)  # 等于
		3: result = velocity_magnitude >= velocity_threshold  # 大于等于
		4: result = velocity_magnitude <= velocity_threshold  # 小于等于

	var result_text = "满足" if result else "不满足"
	var debug_msg = FuseLocalization.translate_format("FUSE_LOG_VELOCITY_CHECK", {
...
func get_description() -> String:
	...
	if node_str.is_empty():
		return FuseLocalization.translate("FUSE_CONDITION_VELOCITY_DESC_NOT_SET")

	var op_symbol = _get_operator_symbol()
	var desc = FuseLocalization.translate_format("FUSE_CONDITION_VELOCITY_FORMAT", {"op": op_symbol, "value": velocity_threshold})
...
func set_parameters(parameters: Dictionary):
	...
	if parameters.has("velocity_threshold"):
...
static func _get_condition_metadata() -> ConditionMetadata:
	...
	metadata.name_key = "FUSE_CONDITION_VELOCITY_NAME"
	metadata.category_key = "FUSE_CATEGORY_PHYSICS"
	metadata.description_key = "FUSE_CONDITION_VELOCITY_DESC"
	metadata.keywords = ["速度", "velocity", "移动", "movement", "magnitude", "比较", "变量", "variable"]
