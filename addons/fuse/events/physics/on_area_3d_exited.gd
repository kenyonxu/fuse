# 文件：addons/fuse/events/on_area_3d_exited.gd
@tool
@icon("res://addons/fuse/icons/builtin/CollisionShape3D.png")
extends BaseEvent
class_name OnArea3DExited

## Event: OnArea3DExited
##
...
func initialize(owner_node: Node) -> void:
	...
	if not _area_node:
		...
		_log_error("[OnArea3DExited] 找不到目标节点: %s" % str(area_node_path))
		return
	...
	if not _area_node is Area3D:
		...
		_log_error("[OnArea3DExited] 目标节点不是 Area3D: %s (类型: %s)" % [_area_node.name, _area_node.get_class()])
		return
...
func _on_body_exited_impl(owner_node: Node, body: Node3D) -> void:
	...
	_log_info("[OnArea3DExited] 触发事件: body '%s' 离开区域" % body.name)

	var context_node = Node.new()
	context_node.name = "Area3DExitedContext"
...
func _on_area_exited_impl(owner_node: Node, area: Area3D) -> void:
	...
	_log_info("[OnArea3DExited] 触发事件: Area3D '%s' 离开区域" % area.name)
...
static func _get_event_metadata() -> EventMetadata:
	...
	metadata.name_key = "FUSE_EVENT_ON_AREA_3D_EXITED_NAME"
	metadata.category_key = "FUSE_EVENT_CATEGORY_PHYSICS"
	metadata.description_key = "FUSE_EVENT_ON_AREA_3D_EXITED_DESC"
	metadata.keywords = ["area", "区域", "exit", "离开", "exited", "collision", "碰撞", "physics", "物理", "3d", "body", "物体"]
