# 文件：addons/fuse/events/on_area_3d_entered.gd
@tool
@icon("res://addons/fuse/icons/builtin/CollisionShape3D.png")
extends BaseEvent
class_name OnArea3DEntered

## Event: OnArea3DEntered
##
...
## 当物体进入 3D 区域时触发的事件
## 设计师必须指定 Area3D 节点的路径（相对于 Trigger 节点）

## 目标 Area3D 节点路径（相对于 Trigger 节点）
...
func _update_resource_name():
	var area_path = area_node_path if not area_node_path.is_empty() else FuseLocalization.translate("FUSE_EVENT_AREA_3D_NOT_SPECIFIED")
	var body_name = FuseLocalization.translate("FUSE_EVENT_AREA_3D_ANY") if target_group.is_empty() else target_group

	if trigger_once_per_body:
		resource_name = FuseLocalization.translate_format("FUSE_EVENT_AREA_3D_RESOURCE_NAME_FIRST", {"body": body_name, "area": area_path})
	else:
		resource_name = FuseLocalization.translate_format("FUSE_EVENT_AREA_3D_RESOURCE_NAME_EVERY", {"body": body_name, "area": area_path})
...
func initialize(owner_node: Node) -> void:
	...
	if not _area_node:
		...
		_log_error("[OnArea3DEntered] 找不到目标节点: %s" % str(area_node_path))
		return

	if not _area_node is Area3D:
		...
		_log_error("[OnArea3DEntered] 目标节点不是 Area3D: %s (类型: %s)" % [_area_node.name, _area_node.get_class()])
		return
	...
	# 连接信号
	# 连接 PhysicsBody3D 信号
	if not _area_node.body_entered.is_connected(body_entered_callable):
		_area_node.body_entered.connect(body_entered_callable)

	if not _area_node.body_exited.is_connected(body_exited_callable):
		_area_node.body_exited.connect(body_exited_callable)

	# 连接 Area3D 信号（用于检测 Area3D 类型的敌人）
	if not _area_node.area_entered.is_connected(area_entered_callable):
		_area_node.area_entered.connect(area_entered_callable)
...
func terminate(owner_node: Node) -> void:
	...
	if _area_node and is_instance_valid(_area_node):
		...
		if owner_id in _signal_connections:
			...
			if _area_node.body_entered.is_connected(callables.body_entered):
				_area_node.body_entered.disconnect(callables.body_entered)

			if _area_node.body_exited.is_connected(callables.body_exited):
				_area_node.body_exited.disconnect(callables.body_exited)

			if _area_node.area_entered.is_connected(callables.area_entered):
				_area_node.area_entered.disconnect(callables.area_entered)
...
func _on_body_entered_impl(owner_node: Node, body: Node3D) -> void:
	if not body:
		return
	...
	_log_info("[OnArea3DEntered] 触发事件: body '%s' 进入区域" % body.name)

	var context_node = Node.new()
	context_node.name = "Area3DEnteredContext"
...
## PhysicsBody3D 离开回调（从已触发列表中移除）
func _on_body_exited_impl(owner_node: Node, body: Node3D) -> void:
	if not body:
		return
...
## 🔧 Area3D 进入回调（用于检测 Area3D 类型的敌人）
func _on_area_entered_impl(owner_node: Node, area: Area3D) -> void:
	if not area:
		return
	...
	_log_info("[OnArea3DEntered] 触发事件: Area3D '%s' 进入区域" % area.name)
...
## 🔧 Area3D 离开回调（从已触发列表中移除）
func _on_area_exited_impl(owner_node: Node, area: Area3D) -> void:
	if not area:
		return
...
func get_description() -> String:
	var area_name = area_node_path if not area_node_path.is_empty() else FuseLocalization.translate("FUSE_EVENT_AREA_3D_NOT_SPECIFIED")
	var body_name = FuseLocalization.translate("FUSE_EVENT_AREA_3D_ANY") if target_group.is_empty() else target_group

	if trigger_once_per_body:
		return FuseLocalization.translate_format("FUSE_EVENT_AREA_3D_RESOURCE_NAME_FIRST", {"body": body_name, "area": area_name})
	else:
		return FuseLocalization.translate_format("FUSE_EVENT_AREA_3D_RESOURCE_NAME_EVERY", {"body": body_name, "area": area_name})

func get_event_type() -> String:
	return "area_3d_entered"
...
func validate() -> Array[String]:
	...
	if area_node_path.is_empty():
		errors.append(FuseLocalization.translate("FUSE_ERROR_AREA_3D_NOT_SPECIFIED"))

	return errors

## 手动触发区域进入事件
func trigger_manually(body: Node3D) -> void:
	_log_debug_localized("FUSE_LOG_EVENT_TRIGGERED", {"event_type": get_event_type()})
	triggered.emit(body)

## 获取当前在区域内的物体列表
func get_bodies_in_area() -> Array[Node3D]:
	if not _area_node:
		return []
...
static func _get_event_metadata() -> EventMetadata:
	...
	metadata.name_key = "FUSE_EVENT_ON_AREA_3D_ENTERED_NAME"
	metadata.category_key = "FUSE_EVENT_CATEGORY_PHYSICS"
	metadata.description_key = "FUSE_EVENT_ON_AREA_3D_ENTERED_DESC"
	metadata.keywords = ["area", "区域", "enter", "进入", "collision", "碰撞", "physics", "物理", "3d", "body", "物体"]
	metadata.builtin_icon = "CollisionShape3D"
	return metadata
