...
- **指令节点**：StatusError（红）/ StatusWarning（黄）主题图标 + 行着色
- **Trigger 节点**：汇总子树问题计数（如 `(2 错误, 1 警告)`），有 error 整行标红
...
当该 Trigger 的指令链存在静态分析问题时，`📊 数据流` 按钮显示**问题计数角标**（如 `2 错误, 1 警告`，有 error 时按钮字体变红）。展开数据流卡片后，底部「问题详情」段按 error / warning 分级列出具体问题（含指令名 + 问题描述），与 Topology 主屏的问题标注保持一致。
