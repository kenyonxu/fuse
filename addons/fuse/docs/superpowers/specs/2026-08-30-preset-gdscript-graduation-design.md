...
**日期：** 2026-08-30
**状态：** 已实现（2026-08-31，M0-M3 验收达标）；**2026-09-01 方向修订：GDScript 生成降级为实验特性**（见下）
...
**方向修订记录（2026-09-01，后于执行中修订记录、优先级最高）：**

出口主线调整为 **AI 交接工件**：拓扑 + System 划分 + preset 供给**用户自己的 AI agent** 编写脱离 Fuse 的代码，Fuse 不代写代码。依据：混合委托模式的生成代码结构性依赖 Fuse 运行时（FuseDelegation + 组件类 + autoload，金样例真实"断奶率"1/28），既非真代码又背上 182 指令的等价性维护包袱；而确定性代码生成器追不上 AI agent 的泛化能力（价值评估文档立论）。部件处置：

- `derive_systems` / `validate_system`（System 推导与校验）——**继续作为主线部件**（"和用户确定需要做什么系统"）
- `gdscript_emitter` + `FuseDelegation` + 金样例——**降级为实验特性**，保留作参考实现；其验证的语义结论（busy 卫语句复刻 SKIP、LOCAL 单 ctx 连续性、两阶段门控、RESTART 降级）是交接工件"语义契约"的素材来源
- handoff bundle（交接工件一键打包）为后继 spec 的主题
