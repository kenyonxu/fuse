...
**组件名必须对照源码**：文中出现的所有指令名（Instruction）、事件名（Event）、条件名（Condition）必须严格来自 `fuse-组件名对照表.md`（从 `addons/fuse/{instructions,events,conditions}/` 源码自动扫描生成），不得臆造。写作前必须先读对照表确认名称准确。引用同一命令时中/英文名须统一。
...
6. **所有指令名/事件名/条件名必须对照 `fuse-组件名对照表.md`，不得臆造。**
7. 该篇若为护城河篇（4/9/12/13/14/15），差异化特性有独立小节并加重笔墨。
8. 篇尾有"下一篇"衔接引导，承接点与上文逻辑闭环。
9. 正文字数落在建议区间 ±15% 内。
10. 所有能力承诺与性能数据（如多线程 2.5x–4x）有文档出处，无未经验证的夸大。
11. 通读校对：无错别字，专有名词大小写全文统一（对照术语表）。
