...
- **宿主行**：容器在前组件在后；容器行 `<path> (<scope_id>)`，组件行 `<path> [<Trigger|MultiEvent|Runner>] · <ago>`；超过 5 秒未上报的宿主名称列灰显
...
一级为场景根。宿主按其节点路径归组：`/root/<名>/...` 前缀的宿主归到对应**附加场景**根，其余归**当前场景**（场景名取运行游戏推送的 `scene` 字段，字段缺失时归入未命名分组）。场景根文本带尾注区分归属：当前场景显示 `<场景名> · 当前`，附加场景显示 `<场景名> · 附加`（当前场景名取自桥推送，场景切换时尾注随之更新）。
...
- 换选自动切换曲线；点击图区标题行的 **✕**、按 **Esc** 或取消选中收起图区；拖动分隔条可调高（默认 100px）
