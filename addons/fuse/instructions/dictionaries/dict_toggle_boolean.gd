...
class_name DictToggleBoolean

## DictToggleBoolean 指令
##
## 切换字典中布尔类型键的值（true -> false, false -> true）。
## 如果键不存在，设置默认值 true。
## 如果值不是布尔类型则报错。
##
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_DICT_TOGGLE_BOOLEAN_NAME"
	metadata.category_key = "FUSE_CATEGORY_DICTIONARIES"
	metadata.description_key = "FUSE_INSTRUCTION_DICT_TOGGLE_BOOLEAN_DESC"
	metadata.keywords = ["字典", "布尔", "切换", "dictionary", "boolean", "toggle", "开关"]
...
## 声明变量读写模式（dict=read 原地修改, key=read）
...
func _update_resource_name():
	...
	resource_name = " ".join(["Dict Toggle", dict_str, "[%s]" % key_str])

## 动态属性设置（支持属性刷新）
func _set(property: StringName, value: Variant) -> bool:
	if property == "dict_variable" or property == "key_value":
		_update_resource_name()
		return false

	if property == "key_from_variable":
		_update_resource_name()
		return false
...
func execute(context: ExecutionContext):
	...
	_log_debug_localized("FUSE_LOG_INSTRUCTION_START", {"instruction": "DictToggleBoolean"})
	...
	# 调试输出
	_log_debug("════════════════════════════════════════════════════")
	_log_debug("📤 DictToggleBoolean 执行:")
	...
	if target_dict.has(key):
		...
		if current_value is bool:
			...
			# 值不是布尔类型，报错
			_log_error_localized("FUSE_ERROR_DICT_VALUE_NOT_BOOLEAN", {"key": str(key), "type": typeof(current_value)})
			set_error_localized("FUSE_ERROR_DICT_VALUE_NOT_BOOLEAN", FuseError.ErrorType.RUNTIME_ERROR, {"key": str(key)})
			finished.emit()
			return
	else:
		# 键不存在，设置默认值 true
	...
	_log_info_localized("FUSE_LOG_DICT_BOOLEAN_TOGGLED", {"dict": dict_variable, "key": str(key), "value": str(new_value)})
...
func get_description() -> String:
	...
	return "Dict Toggle: %s[%s]" % [dict_str, key_str]

## 重置指令状态
func reset():
	super.reset()
	_log_debug_localized("FUSE_LOG_DICT_TOGGLE_RESET", {})
