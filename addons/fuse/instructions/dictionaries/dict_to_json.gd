@tool
@icon("res://addons/fuse/icons/builtin/JSON.svg")
extends BaseInstruction
class_name DictToJson

## DictToJson 指令
##
## 将字典转换为 JSON 字符串并存储到目标变量。
## 支持格式化选项（pretty_print）控制缩进。
##
...
# 缩进空格数（仅在 pretty_print 为 true 时使用）
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_DICT_TO_JSON_NAME"
	metadata.category_key = "FUSE_CATEGORY_DICTIONARIES"
	metadata.description_key = "FUSE_INSTRUCTION_DICT_TO_JSON_DESC"
	metadata.keywords = ["字典", "JSON", "转换", "dictionary", "dict", "json", "serialize", "序列化"]
	metadata.builtin_icon = "JSON"
	return metadata
...
## 声明变量读写模式（dict=read, target=write json 字符串）
...
func _get_property_list() -> Array[Dictionary]:
	...
	# 只在 pretty_print 为 true 时显示缩进设置
	if pretty_print:
		properties.append({
			name = "indent_spaces",
			type = TYPE_INT,
...
func _update_resource_name():
	...
	# 格式信息
	if pretty_print:
		format_str = "(%d spaces)" % indent_spaces
	else:
		format_str = "(compact)"
	...
	resource_name = " ".join(["Dict To JSON", dict_str, format_str, "→", target_str])

## 执行指令
func execute(context: ExecutionContext):
	_start_execution(context)

	_log_debug_localized("FUSE_LOG_INSTRUCTION_START", {"instruction": "DictToJson"})
	...
	if pretty_print:
		json_string = JSON.stringify(target_dict, "    ".repeat(max(0, indent_spaces) / 4))
	else:
		json_string = JSON.stringify(target_dict)

	# 检查转换结果
	if json_string.is_empty() and not target_dict.is_empty():
		_log_error_localized("FUSE_ERROR_DICT_TO_JSON_FAILED", {})
		set_error_localized("FUSE_ERROR_DICT_TO_JSON_FAILED", FuseError.ErrorType.EXECUTION_ERROR, {})
		finished.emit()
		return

	# 记录日志
	_log_info_localized("FUSE_LOG_DICT_TO_JSON", {"dict": dict_variable, "target": target_variable})
...
func _validate_property(property: Dictionary) -> void:
	...
	# 格式化相关属性
	if not pretty_print:
		if property.name == "indent_spaces":
			property.usage = PROPERTY_USAGE_NO_EDITOR
...
func get_description() -> String:
	...
	var format_str = "pretty" if pretty_print else "compact"

	return "Dict To JSON: %s (%s) -> %s" % [dict_str, format_str, target_str]

## 重置指令状态
func reset():
	super.reset()
	_log_debug_localized("FUSE_LOG_DICT_TO_JSON_RESET", {})
