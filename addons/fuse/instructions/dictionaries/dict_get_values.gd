@tool
@icon("res://addons/fuse/icons/builtin/Enumeration.svg")
extends BaseInstruction
class_name DictGetValues

## DictGetValues 指令
##
## 从字典中获取所有值组成的数组并存储到目标变量。
## 使用 dict.values() 方法。
##
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_DICT_GET_VALUES_NAME"
	metadata.category_key = "FUSE_CATEGORY_DICTIONARIES"
	metadata.description_key = "FUSE_INSTRUCTION_DICT_GET_VALUES_DESC"
	metadata.keywords = ["字典", "获取", "所有值", "数组", "dictionary", "dict", "values", "array"]
	metadata.builtin_icon = "Enumeration"
	return metadata
...
## 声明变量读写模式（dict=read, target=write values 数组）
...
func _update_resource_name():
	...
	resource_name = " ".join(["Dict Get Values", dict_str, "→", target_str])

## 动态属性设置（支持属性刷新）
func _set(property: StringName, value: Variant) -> bool:
	if property in ["dict_variable", "target_variable"]:
		_update_resource_name()
		return false
...
func execute(context: ExecutionContext):
	...
	_log_debug_localized("FUSE_LOG_INSTRUCTION_START", {"instruction": "DictGetValues"})
	...
	# 获取所有值
	var values: Array = target_dict.values()
	_log_info_localized("FUSE_LOG_DICT_GET_VALUES", {"count": values.size(), "target": target_variable})

	# 存储到目标变量
	_set_target_variable(context, values)
...
func get_description() -> String:
	...
	return "Dict Get Values: %s → %s" % [dict_str, target_str]

## 重置指令状态
func reset():
	super.reset()
	_log_debug_localized("FUSE_LOG_DICT_GET_VALUES_RESET", {})
