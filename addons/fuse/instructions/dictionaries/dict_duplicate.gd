...
class_name DictDuplicate

## DictDuplicate 指令
##
## 创建字典的深拷贝或浅拷贝并存储到目标变量。
## 使用 dict.duplicate(deep) 方法实现拷贝。
## deep_copy = true 时递归复制嵌套对象（深拷贝）。
## deep_copy = false 时只复制顶层（浅拷贝）。
##
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_DICT_DUPLICATE_NAME"
	metadata.category_key = "FUSE_CATEGORY_DICTIONARIES"
	metadata.description_key = "FUSE_INSTRUCTION_DICT_DUPLICATE_DESC"
	metadata.keywords = ["字典", "拷贝", "复制", "深拷贝", "浅拷贝", "dictionary", "dict", "duplicate", "copy", "deep", "clone"]
...
## 声明变量读写模式（dict=read, target=write 副本）
...
func _update_resource_name():
	...
	# 拷贝类型
	if deep_copy:
		copy_type_str = FuseLocalization.translate("FUSE_INSTRUCTION_DICT_DUPLICATE_DEEP")
	else:
		copy_type_str = FuseLocalization.translate("FUSE_INSTRUCTION_DICT_DUPLICATE_SHALLOW")
	...
	resource_name = " ".join(["Dict Duplicate", dict_str, "(%s)" % copy_type_str, "→", target_str])

## 动态属性设置（支持属性刷新）
func _set(property: StringName, value: Variant) -> bool:
	if property in ["dict_variable", "deep_copy", "target_variable"]:
		_update_resource_name()
		return false
...
func execute(context: ExecutionContext):
	...
	_log_debug_localized("FUSE_LOG_INSTRUCTION_START", {"instruction": "DictDuplicate"})
	...
	# 执行拷贝
	var copied_dict: Dictionary = target_dict.duplicate(deep_copy)

	# 记录日志
	_log_info_localized("FUSE_LOG_DICT_DUPLICATED", {
		"source": dict_variable,
		"target": target_variable,
		"deep": "true" if deep_copy else "false"
	})

	# 存储到目标变量
	_set_target_variable(context, copied_dict)
...
func get_description() -> String:
	...
	var copy_type = "Deep" if deep_copy else "Shallow"
	var target_str = target_variable if not target_variable.is_empty() else "No Target"

	return "Dict Duplicate: %s (%s) -> %s" % [dict_str, copy_type, target_str]

## 重置指令状态
func reset():
	super.reset()
	_log_debug_localized("FUSE_LOG_DICT_DUPLICATE_RESET", {})
