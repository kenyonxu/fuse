...
class_name DictGetKeys

## DictGetKeys 指令
##
## 获取字典中所有键组成的数组并存储到目标变量。
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_DICT_GET_KEYS_NAME"
	metadata.category_key = "FUSE_CATEGORY_DICTIONARIES"
	metadata.description_key = "FUSE_INSTRUCTION_DICT_GET_KEYS_DESC"
	metadata.keywords = ["字典", "获取", "键", "数组", "dictionary", "dict", "keys", "array"]
...
## 声明变量读写模式（dict=read, target=write keys 数组）
...
func _update_resource_name():
	...
	resource_name = " ".join(["Dict Get Keys", dict_str, "→", target_str])
...
func execute(context: ExecutionContext):
	...
	_log_debug_localized("FUSE_LOG_INSTRUCTION_START", {"instruction": "DictGetKeys"})
	...
	# 获取所有键
	var keys: Array = target_dict.keys()

	_log_info_localized("FUSE_LOG_DICT_GET_KEYS", {"dict": dict_variable, "count": keys.size(), "target": target_variable})

	# 存储到目标变量
	_set_target_variable(context, keys)
...
func get_description() -> String:
	...
	return "Dict Get Keys: %s → %s" % [dict_str, target_str]

## 重置指令状态
func reset():
	super.reset()
	_log_debug_localized("FUSE_LOG_DICT_GET_KEYS_RESET", {})
