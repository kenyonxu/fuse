...
## 操作类型
var operation: OperationType = OperationType.ADD:
	set(value):
		if operation != value:
			operation = value
			_update_resource_name()
...
## 修改量是否来自变量
var use_amount_from_variable: bool = false:
	set(value):
		if use_amount_from_variable != value:
			use_amount_from_variable = value
			_update_resource_name()
			notify_property_list_changed()

## 修改量源变量名（当 use_amount_from_variable = true 时使用）
var amount_from_variable: String = ""

## 修改量源变量作用域
var amount_from_variable_scope: BaseVariable.VariableScope = BaseVariable.VariableScope.LOCAL:
	set(value):
		if amount_from_variable_scope != value:
			amount_from_variable_scope = value
			_update_resource_name()
			notify_property_list_changed()

## 修改量源作用域来源（仅当 amount_from_variable_scope == SCOPE 时使用）
var amount_scope_source: ScopeSource = ScopeSource.NEAREST:
	set(value):
		if amount_scope_source != value:
			amount_scope_source = value
			_update_resource_name()
			notify_property_list_changed()

## 修改量源自定义作用域 ID
var amount_custom_scope_id: String = "":
	set(value):
		if amount_custom_scope_id != value:
			amount_custom_scope_id = value
			_update_resource_name()

## 修改量源目标节点路径
var amount_target_node_path: NodePath = NodePath(""):
	set(value):
		if amount_target_node_path != value:
			amount_target_node_path = value
			_update_resource_name()
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_DICT_MODIFY_NUMBER_NAME"
	metadata.category_key = "FUSE_CATEGORY_DICTIONARIES"
	metadata.description_key = "FUSE_INSTRUCTION_DICT_MODIFY_NUMBER_DESC"
	metadata.keywords = ["字典", "数值", "修改", "加法", "减法", "dictionary", "number", "modify", "add", "subtract"]
...
## 声明变量读写模式（dict=read 原地修改, key/amount=read）
func get_variable_modes() -> Array[Dictionary]:
	return [
		{"name": "dict_variable", "mode": "read"},
		{"name": "key_from_variable", "mode": "read"},
		{"name": "amount_from_variable", "mode": "read"},
	]
...
func _get_property_list() -> Array[Dictionary]:
	...
	# 修改量或修改量源变量
	if use_amount_from_variable:
		properties.append({
			name = "amount_from_variable",
			type = TYPE_STRING,
			hint = PROPERTY_HINT_NONE,
			usage = PROPERTY_USAGE_DEFAULT | PROPERTY_USAGE_SCRIPT_VARIABLE
		})
		...
		properties.append({
			name = "amount_from_variable_scope",
			type = TYPE_INT,
			hint = PROPERTY_HINT_ENUM,
			hint_string = "Local,Scope,Global",
			usage = PROPERTY_USAGE_DEFAULT | PROPERTY_USAGE_SCRIPT_VARIABLE
		...
		# 只在 amount_from_variable_scope == SCOPE 时显示 ScopeSource 配置
		if amount_from_variable_scope == BaseVariable.VariableScope.SCOPE:
			properties.append({
				name = "Amount Scope Configuration",
				type = TYPE_NIL,
				hint = PROPERTY_HINT_NONE,
				usage = PROPERTY_USAGE_CATEGORY
			})
			...
			# 根据作用域来源添加额外属性
			if amount_scope_source == ScopeSource.CUSTOM_ID:
				properties.append({
					name = "amount_custom_scope_id",
					type = TYPE_STRING,
					hint = PROPERTY_HINT_NONE,
					usage = PROPERTY_USAGE_DEFAULT | PROPERTY_USAGE_SCRIPT_VARIABLE
				})
			elif amount_scope_source == ScopeSource.TARGET_NODE:
				properties.append({
					name = "amount_target_node_path",
					type = TYPE_NODE_PATH,
					hint = PROPERTY_HINT_NONE,
					usage = PROPERTY_USAGE_DEFAULT | PROPERTY_USAGE_SCRIPT_VARIABLE
				})
...
func _update_resource_name():
	...
	# 修改量信息
	if use_amount_from_variable:
		if amount_from_variable.is_empty():
			amount_str = "(no amount var)"
		else:
			amount_str = "$%s" % amount_from_variable
	else:
		amount_str = str(amount)

	resource_name = " ".join(["Dict Modify", dict_str, "[%s]" % key_str, op_str, amount_str])

## 动态属性设置（支持属性刷新）
func _set(property: StringName, value: Variant) -> bool:
	if property == "dict_variable" or property == "key_value" or property == "operation" or property == "amount":
		_update_resource_name()
		return false

	if property == "key_from_variable" or property == "amount_from_variable":
		_update_resource_name()
		return false
...
func execute(context: ExecutionContext):
	...
	_log_debug_localized("FUSE_LOG_INSTRUCTION_START", {"instruction": "DictModifyNumber"})
	...
	# 获取修改量
	var modify_amount: float
	if use_amount_from_variable:
		# 从变量获取修改量
		if amount_from_variable.is_empty():
			_log_error_localized("FUSE_ERROR_DICT_MODIFY_AMOUNT_EMPTY", {})
			set_error_localized("FUSE_ERROR_DICT_MODIFY_AMOUNT_EMPTY", FuseError.ErrorType.VALIDATION_ERROR, {})
			finished.emit()
			return

		var amount_value = _get_variable_value(context, amount_from_variable, amount_from_variable_scope, amount_scope_source, amount_custom_scope_id, amount_target_node_path)
		...
		# 验证修改量是否为数值类型
		if not _is_numeric_type(amount_value):
			_log_error_localized("FUSE_ERROR_DICT_MODIFY_AMOUNT_NOT_NUMBER", {"type": _get_type_name(typeof(amount_value))})
			set_error_localized("FUSE_ERROR_DICT_MODIFY_AMOUNT_NOT_NUMBER", FuseError.ErrorType.VALIDATION_ERROR, {"type": _get_type_name(typeof(amount_value))})
			finished.emit()
			return

		modify_amount = _to_float(amount_value)
	else:
		# 使用直接值
	...
	if not target_dict.has(key):
		...
		set_error_localized("FUSE_ERROR_DICT_KEY_NOT_FOUND", FuseError.ErrorType.VALIDATION_ERROR, {"key": str(key)})
		finished.emit()
		return
	...
	if not _is_numeric_type(current_value):
		...
		set_error_localized("FUSE_ERROR_DICT_VALUE_NOT_NUMBER", FuseError.ErrorType.VALIDATION_ERROR, {"key": str(key), "type": _get_type_name(typeof(current_value))})
		finished.emit()
		return
	...
	match operation:
		OperationType.ADD:
			new_value = old_value + modify_amount
		OperationType.SUBTRACT:
			new_value = old_value - modify_amount
	...
	# 调试输出
	_log_debug("════════════════════════════════════════════════════")
	_log_debug("📤 DictModifyNumber 执行:")
	_log_debug("  • 目标字典: '%s'" % dict_variable)
	_log_debug("  • 作用域: %s" % _get_scope_name_for_log())
	_log_debug("  • 键: %s (类型: %s)" % [str(key), typeof(key)])
	_log_debug("  • 操作: %s" % ("+" if operation == OperationType.ADD else "-"))
	_log_debug("  • 修改量: %s" % str(modify_amount))
	_log_debug("  • 原值: %s -> 新值: %s" % [str(old_value), str(target_dict[key])])
	_log_debug("════════════════════════════════════════════════════")

	var op_symbol := "+=" if operation == OperationType.ADD else "-="
	_log_info_localized("FUSE_LOG_DICT_NUMBER_MODIFIED", {
		"dict": dict_variable,
		"key": str(key),
		"op": op_symbol,
		"amount": str(modify_amount),
		"old": str(old_value),
...
func validate() -> Array[String]:
	...
	if use_amount_from_variable:
		if amount_from_variable.is_empty():
			errors.append(FuseLocalization.translate("FUSE_ERROR_DICT_MODIFY_AMOUNT_EMPTY"))
		...
		if amount_from_variable_scope == BaseVariable.VariableScope.SCOPE:
			...
			# 验证 ScopeSource 参数
			var amount_utils_scope_source = amount_scope_source as VariableScopeUtils.ScopeSource
			errors.append_array(VariableScopeUtils.validate_scope_source_params(
				amount_utils_scope_source,
				amount_custom_scope_id,
				amount_target_node_path
			))
...
func _validate_property(property: Dictionary) -> void:
	...
	if use_amount_from_variable:
		...
		if property.name in ["amount_from_variable", "amount_from_variable_scope", "amount_scope_source", "amount_custom_scope_id", "amount_target_node_path"]:
			property.usage = PROPERTY_USAGE_NO_EDITOR

	# 修改量作用域相关属性
	if use_amount_from_variable:
		if amount_from_variable_scope == BaseVariable.VariableScope.SCOPE:
			if property.name == "amount_scope_source":
				return  # 始终显示
			elif property.name == "amount_custom_scope_id":
				if amount_scope_source != ScopeSource.CUSTOM_ID:
					property.usage = PROPERTY_USAGE_NO_EDITOR
			elif property.name == "amount_target_node_path":
				if amount_scope_source != ScopeSource.TARGET_NODE:
					property.usage = PROPERTY_USAGE_NO_EDITOR
		else:
			if property.name in ["amount_scope_source", "amount_custom_scope_id", "amount_target_node_path"]:
				property.usage = PROPERTY_USAGE_NO_EDITOR
...
func get_description() -> String:
	...
	if use_amount_from_variable:
		amount_str = "$%s" % amount_from_variable if not amount_from_variable.is_empty() else "(no var)"
	else:
		amount_str = str(amount)

	return "Dict Modify: %s[%s] %s %s" % [dict_str, key_str, op_str, amount_str]

## 重置指令状态
func reset():
	super.reset()
	_log_debug_localized("FUSE_LOG_DICT_MODIFY_NUMBER_RESET", {})
