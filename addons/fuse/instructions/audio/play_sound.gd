...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_PLAY_SOUND_NAME"
	metadata.category_key = "FUSE_CATEGORY_AUDIO"
	metadata.description_key = "FUSE_INSTRUCTION_PLAY_SOUND_DESC"
	metadata.keywords = ["play", "sound", "audio", "sfx", "播放", "音效", "声音"]
...
func _update_resource_name():
	...
	parts.append(FuseLocalization.translate("FUSE_INSTRUCTION_PLAY_SOUND_RESOURCE_BASE"))

	if not sound_path.is_empty():
		parts.append("'%s'" % FuseNodeUtils.get_path_display_name(sound_path))
	else:
		parts.append(FuseLocalization.translate("FUSE_INSTRUCTION_PLAY_SOUND_RESOURCE_NO_SOUND"))
...
func execute(context: ExecutionContext):
	...
	if sound_path.is_empty():
		_log_error_localized("FUSE_ERROR_SOUND_PATH_EMPTY", {})
		set_error_localized("FUSE_ERROR_SOUND_PATH_EMPTY", FuseError.ErrorType.VALIDATION_ERROR, {})
		finished.emit()
		return
	...
	if scene_tree and scene_tree.current_scene:
		...
		# 播放音频
		audio_player.play()
		_log_info_localized("FUSE_INSTRUCTION_PLAY_SOUND_LOG_PLAYING", {"file": FuseNodeUtils.get_path_display_name(sound_path)})
	else:
...
func validate() -> Array[String]:
	...
	if sound_path.is_empty():
		errors.append(FuseLocalization.translate("FUSE_INSTRUCTION_PLAY_SOUND_VALIDATE_PATH_EMPTY"))
...
func get_description() -> String:
	...
	var file_str = FuseNodeUtils.get_path_display_name(sound_path) if not sound_path.is_empty() else FuseLocalization.translate("FUSE_INSTRUCTION_PLAY_SOUND_RESOURCE_NO_SOUND")
	return FuseLocalization.translate_format("FUSE_INSTRUCTION_PLAY_SOUND_DESC_FILE", {"file": file_str, "options": options_str})
