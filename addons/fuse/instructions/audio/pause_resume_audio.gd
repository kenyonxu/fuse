...
func _update_resource_name():
	...
	match target_mode:
		TargetMode.SPECIFIC_PLAYER:
			parts.append(_get_specific_player_display())
		TargetMode.BY_BUS:
...
func _get_specific_player_display() -> String:
	if use_variable_for_target:
		if target_variable.is_empty():
			return FuseLocalization.translate("FUSE_INSTRUCTION_PAUSE_RESUME_AUDIO_TARGET_NO_PLAYER")
		...
		return FuseLocalization.translate_format("FUSE_INSTRUCTION_PAUSE_RESUME_AUDIO_TARGET_SPECIFIC", {"player": "%s [%s]" % [target_variable, scope_str]})
	if not target_player.is_empty():
		return FuseLocalization.translate_format("FUSE_INSTRUCTION_PAUSE_RESUME_AUDIO_TARGET_SPECIFIC", {"player": str(target_player)})
	return FuseLocalization.translate("FUSE_INSTRUCTION_PAUSE_RESUME_AUDIO_TARGET_NO_PLAYER")
...
func _set(property: StringName, value: Variant) -> bool:
	if property in ["target_mode", "use_variable_for_target", "target_scope", "target_scope_source"]:
		set(property, value)
		notify_property_list_changed()
		_update_resource_name()
		return true
...
func get_description() -> String:
	...
	match target_mode:
		TargetMode.SPECIFIC_PLAYER:
			...
			if not use_variable_for_target and target_player.is_empty():
				target_desc = FuseLocalization.translate("FUSE_INSTRUCTION_PAUSE_RESUME_AUDIO_DESC_SPECIFIED")
		TargetMode.BY_BUS:
