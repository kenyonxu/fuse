...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_GET_GROUP_COUNT_NAME"
	metadata.category_key = "FUSE_CATEGORY_NODE_OPERATIONS"
	metadata.description_key = "FUSE_INSTRUCTION_GET_GROUP_COUNT_DESC"
	metadata.keywords = ["group", "count", "size", "nodes", "组", "数量", "计数", "节点"]
...
func _update_resource_name():
	...
	parts.append(FuseLocalization.translate("FUSE_INSTRUCTION_GET_GROUP_COUNT_RESOURCE_BASE"))

	if group_name.is_empty():
		parts.append(FuseLocalization.translate("FUSE_INSTRUCTION_GET_GROUP_COUNT_NO_GROUP"))
	else:
		parts.append("'%s'" % group_name)

	if result_variable.is_empty():
		parts.append(FuseLocalization.translate("FUSE_INSTRUCTION_GET_GROUP_COUNT_NO_VARIABLE"))
	else:
		var scope_str = _get_scope_source_string()
		parts.append(FuseLocalization.translate_format(
			"FUSE_INSTRUCTION_GET_GROUP_COUNT_VAR_TEMPLATE",
...
func execute(context: ExecutionContext):
	...
	# 记录结果
	_log_info_localized("FUSE_LOG_GET_GROUP_COUNT_RESULT", {
		"group": group_name,
		"count": count
	})
...
func _save_result(context: ExecutionContext, value: int):
	...
	if success:
		var scope_str = _get_scope_source_string()
		_log_info_localized("FUSE_LOG_GET_GROUP_COUNT_SAVED", {
			"var": result_variable,
			"scope": scope_str,
			"count": value
		})
	else:
		_log_error_localized("FUSE_LOG_GET_GROUP_COUNT_SAVE_FAILED", {
			"var": result_variable,
...
func get_description() -> String:
	...
	return FuseLocalization.translate_format(
		"FUSE_INSTRUCTION_GET_GROUP_COUNT_DESC_FORMAT",
		{
			"group": group_name,
			"var": result_variable,
			"scope": scope_str
