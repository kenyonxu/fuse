...
## 获取指定节点所有子节点的位置指令
##
## 从指定节点获取所有子节点（支持递归），并将位置数组保存到指定变量中。
## 自动区分 Vector2（Node2D）和 Vector3（Node3D）。
##
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_GET_ALL_CHILDREN_POSITION_NAME"
	metadata.category_key = "FUSE_CATEGORY_NODE_OPERATIONS"
	metadata.description_key = "FUSE_INSTRUCTION_GET_ALL_CHILDREN_POSITION_DESC"
	metadata.keywords = ["children", "position", "nodes", "get", "child", "子节点", "位置", "坐标", "获取"]
...
func _update_resource_name():
	...
	# 递归标记
	if recursive:
		parts.append("[%s]" % FuseLocalization.translate("FUSE_INSTRUCTION_GET_ALL_CHILDREN_POSITION_RECURSIVE"))

	# 坐标空间
	var space_str = "global" if use_global_position else "local"
	parts.append("[%s]" % space_str)

	parts.append("→")

	# 保存变量部分
	if result_variable.is_empty():
		parts.append(FuseLocalization.translate("FUSE_INSTRUCTION_GET_ALL_CHILDREN_POSITION_NO_VARIABLE"))
	else:
...
func execute(context: ExecutionContext):
	...
	# 收集位置
	var positions: Array = []
	var node_2d_count: int = 0
	var node_3d_count: int = 0
	...
	if positions.is_empty():
		_log_info_localized("FUSE_LOG_GET_ALL_CHILDREN_POSITION_EMPTY", {"node": target.name})
	else:
		_log_info_localized("FUSE_LOG_GET_ALL_CHILDREN_POSITION_FOUND", {
			"node": target.name,
			"count": positions.size(),
			"node2d": node_2d_count,
			"node3d": node_3d_count
		})

	# 保存结果到变量
	_save_result(context, positions)
...
func _save_result(context: ExecutionContext, value: Array):
	...
	if success:
		...
		_log_info_localized("FUSE_LOG_GET_ALL_CHILDREN_POSITION_SAVED", {
			"var": result_variable,
			"scope": scope_str,
			"count": value.size()
		})
	else:
		_log_error_localized("FUSE_LOG_GET_ALL_CHILDREN_POSITION_SAVE_FAILED", {
			"var": result_variable,
...
func get_description() -> String:
	...
	if use_variable:
		if target_variable.is_empty():
			...
			var scope_str = VariableScopeUtils.enum_to_string(target_scope).to_upper()
			target_desc = FuseLocalization.translate_format("FUSE_INSTRUCTION_GET_ALL_CHILDREN_POSITION_TARGET_VARIABLE", {
				"variable": "%s [%s]" % [target_variable, scope_str]
			})
	else:
		if target_node.is_empty():
			target_desc = FuseLocalization.translate("FUSE_INSTRUCTION_GET_ALL_CHILDREN_POSITION_NO_TARGET")
		else:
			target_desc = FuseLocalization.translate_format("FUSE_INSTRUCTION_GET_ALL_CHILDREN_POSITION_TARGET_NODE", {
				"node": _get_node_display_name(target_node)
			})

	# 获取保存描述
	if result_variable.is_empty():
		save_desc = FuseLocalization.translate("FUSE_INSTRUCTION_GET_ALL_CHILDREN_POSITION_NO_VARIABLE")
	else:
		var scope_str = VariableScopeUtils.enum_to_string(result_scope).to_upper()
		save_desc = FuseLocalization.translate_format("FUSE_INSTRUCTION_GET_ALL_CHILDREN_POSITION_SAVE_VARIABLE", {
			"variable": "%s [%s]" % [result_variable, scope_str]
		})

	var recursive_str = FuseLocalization.translate("FUSE_INSTRUCTION_GET_ALL_CHILDREN_POSITION_RECURSIVE") if recursive else ""
	var space_str = "global" if use_global_position else "local"

	return FuseLocalization.translate_format("FUSE_INSTRUCTION_GET_ALL_CHILDREN_POSITION_DESC_FORMAT", {
		"target": target_desc,
		"variable": save_desc,
