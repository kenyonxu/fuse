...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_GET_CHILD_COUNT_NAME"
	metadata.category_key = "FUSE_CATEGORY_NODE_OPERATIONS"
	metadata.description_key = "FUSE_INSTRUCTION_GET_CHILD_COUNT_DESC"
	metadata.keywords = ["child", "count", "children", "size", "子节点", "数量", "计数", "孩子"]
...
func _update_resource_name():
	...
	parts.append(FuseLocalization.translate("FUSE_INSTRUCTION_GET_CHILD_COUNT_RESOURCE_BASE"))

	if target_node.is_empty():
		parts.append(FuseLocalization.translate("FUSE_INSTRUCTION_GET_CHILD_COUNT_NO_TARGET"))
	else:
		parts.append("→ %s" % _get_node_display_name(target_node))

	if result_variable.is_empty():
		parts.append(FuseLocalization.translate("FUSE_INSTRUCTION_GET_CHILD_COUNT_NO_VARIABLE"))
	else:
		var scope_str = _get_scope_source_string()
		parts.append(FuseLocalization.translate_format(
			"FUSE_INSTRUCTION_GET_CHILD_COUNT_VAR_TEMPLATE",
...
func execute(context: ExecutionContext):
	...
	# 记录结果
	_log_info_localized("FUSE_LOG_GET_CHILD_COUNT_RESULT", {
		"node": target.name,
		"count": count
	})

	# 保存结果到变量
	_save_result(context, count)

	_on_execution_completed()

## 保存结果到变量
func _save_result(context: ExecutionContext, value: int):
	var success = false
	match result_scope:
		BaseVariable.VariableScope.LOCAL:
	...
	if success:
		var scope_str = _get_scope_source_string()
		_log_info_localized("FUSE_LOG_GET_CHILD_COUNT_SAVED", {
			"var": result_variable,
			"scope": scope_str,
			"count": value
		})
	else:
		_log_error_localized("FUSE_LOG_GET_CHILD_COUNT_SAVE_FAILED", {
			"var": result_variable,
...
func get_description() -> String:
	var scope_str = _get_scope_source_string()
	var target_str = FuseLocalization.translate("FUSE_INSTRUCTION_GET_CHILD_COUNT_NO_TARGET") if target_node.is_empty() else _get_node_display_name(target_node)

	return FuseLocalization.translate_format(
		"FUSE_INSTRUCTION_GET_CHILD_COUNT_DESC_FORMAT",
		{
			"target": target_str,
			"var": result_variable,
			"scope": scope_str
