...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_GET_LAST_CHILD_NAME"
	metadata.category_key = "FUSE_CATEGORY_NODE_OPERATIONS"
	metadata.description_key = "FUSE_INSTRUCTION_GET_LAST_CHILD_DESC"
	metadata.keywords = ["child", "last", "get", "children", "子节点", "最后", "获取"]
...
func _update_resource_name():
	...
	parts.append(FuseLocalization.translate("FUSE_INSTRUCTION_GET_LAST_CHILD_RESOURCE_BASE"))

	if target_node.is_empty():
		parts.append(FuseLocalization.translate("FUSE_INSTRUCTION_GET_LAST_CHILD_NO_TARGET"))
	else:
		parts.append("→ %s" % _get_node_display_name(target_node))

	if result_variable.is_empty():
		parts.append(FuseLocalization.translate("FUSE_INSTRUCTION_GET_LAST_CHILD_NO_VARIABLE"))
	else:
		var scope_str = _get_scope_source_string()
		parts.append(FuseLocalization.translate_format(
			"FUSE_INSTRUCTION_GET_LAST_CHILD_VAR_TEMPLATE",
...
func execute(context: ExecutionContext):
	...
	if include_internal:
		...
		if child_count > 0:
			child = target.get_child(child_count - 1)
	else:
		child = _get_last_non_internal_child(target)
	...
	# 记录结果
	_log_info_localized("FUSE_LOG_GET_LAST_CHILD_RESULT", {
		"node": target.name,
		"child": child.name
	})
...
## 获取最后一个非内部子节点
func _get_last_non_internal_child(parent: Node) -> Node:
...
func _save_result(context: ExecutionContext, value: String):
	...
	if success:
		var scope_str = _get_scope_source_string()
		_log_info_localized("FUSE_LOG_GET_LAST_CHILD_SAVED", {
			"var": result_variable,
			"scope": scope_str,
			"path": value
		})
	else:
		_log_error_localized("FUSE_LOG_GET_LAST_CHILD_SAVE_FAILED", {
			"var": result_variable,
...
func get_description() -> String:
	var scope_str = _get_scope_source_string()
	var target_str = FuseLocalization.translate("FUSE_INSTRUCTION_GET_LAST_CHILD_NO_TARGET") if target_node.is_empty() else _get_node_display_name(target_node)

	return FuseLocalization.translate_format(
		"FUSE_INSTRUCTION_GET_LAST_CHILD_DESC_FORMAT",
		{
			"target": target_str,
			"var": result_variable,
			"scope": scope_str
