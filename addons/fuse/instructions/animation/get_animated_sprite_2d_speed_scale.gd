...
class_name GetAnimatedSprite2DSpeedScale

## 获取 AnimatedSprite2D 播放速度缩放
##
## 读取 AnimatedSprite2D 节点的 speed_scale 属性值并保存到变量。
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_GET_ANIMATED_SPRITE_2D_SPEED_SCALE_NAME"
	metadata.category_key = "FUSE_CATEGORY_ANIMATION"
	metadata.description_key = "FUSE_INSTRUCTION_GET_ANIMATED_SPRITE_2D_SPEED_SCALE_DESC"
	metadata.keywords = ["animatedsprite2d", "speed_scale", "speed", "get", "速度", "获取", "动画", "精灵"]
...
func _update_resource_name():
	...
	if use_variable_for_target:
		if target_variable.is_empty():
			target_str = FuseLocalization.translate("FUSE_INSTRUCTION_GET_ANIMATED_SPRITE_2D_SPEED_SCALE_NO_TARGET")
		else:
		...
		target_str = _get_node_display_name(target_node) if not target_node.is_empty() else FuseLocalization.translate("FUSE_INSTRUCTION_GET_ANIMATED_SPRITE_2D_SPEED_SCALE_NO_TARGET")
	...
	resource_name = FuseLocalization.translate_format("FUSE_INSTRUCTION_GET_ANIMATED_SPRITE_2D_SPEED_SCALE_RESOURCE_NAME", {
		"target": target_str,
		"variable": save_str
	})
...
func get_description() -> String:
	...
	return FuseLocalization.translate_format("FUSE_INSTRUCTION_GET_ANIMATED_SPRITE_2D_SPEED_SCALE_DESC_FORMAT", {
		"target": target_str,
		"variable": save_str
	})
...
func execute(context: ExecutionContext):
	...
	_log_info_localized("FUSE_LOG_GET_ANIMATED_SPRITE_2D_SPEED_SCALE", {"node": node.name, "value": value})
