...
func execute(context: ExecutionContext):
	...
	if use_variable:
		...
		if var_value == null and not VariableOperations.has_variable(context, scale_variable, scale_scope):
			_log_error_localized("FUSE_ERROR_VAR_NOT_FOUND", {"name": scale_variable})
			set_error_localized("FUSE_ERROR_VAR_NOT_FOUND", FuseError.ErrorType.VALIDATION_ERROR, {"name": scale_variable})
			finished.emit()
			return
