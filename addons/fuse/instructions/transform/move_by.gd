...
func _update_resource_name():
	...
	if use_variable_for_target:
		if target_variable.is_empty():
			target_str = FuseLocalization.translate("FUSE_INSTRUCTION_MOVE_BY_NO_NODE")
		else:
		...
		target_str = _get_node_display_name(target_node) if not target_node.is_empty() else FuseLocalization.translate("FUSE_INSTRUCTION_MOVE_BY_NO_NODE")
...
func get_description() -> String:
	...
	return FuseLocalization.translate_format("FUSE_INSTRUCTION_MOVE_BY_DESC_FORMAT", {"node": node_str, "offset": offset_desc, "space": space_str})
