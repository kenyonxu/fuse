...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_GET_DELTA_TIME_NAME"
	metadata.category_key = "FUSE_CATEGORY_TIME"
	metadata.description_key = "FUSE_INSTRUCTION_GET_DELTA_TIME_DESC"
	metadata.keywords = ["delta", "time", "frame", "delta时间", "帧", "时间"]
...
func _update_resource_name():
	...
	parts.append(FuseLocalization.translate("FUSE_INSTRUCTION_GET_DELTA_TIME_RESOURCE"))

	if not save_to_variable.is_empty():
		var scope_str = _get_scope_source_string()
		parts.append(FuseLocalization.translate_format(
			"FUSE_INSTRUCTION_GET_DELTA_TIME_TO_VARIABLE",
			{"type": scope_str, "name": save_to_variable}
		))
	else:
		parts.append(FuseLocalization.translate_format(
			"FUSE_INSTRUCTION_GET_DELTA_TIME_NOT_SPECIFIED",
			{"text": FuseLocalization.translate("FUSE_VARIABLE_NOT_SPECIFIED")}
		))
...
func execute(context: ExecutionContext):
	...
	# 从执行上下文获取 delta 时间
	var delta_time = context.delta_time

	# 验证 delta 时间有效性
	if is_nan(delta_time) or is_inf(delta_time) or delta_time < 0.0:
		_log_error_localized("FUSE_ERROR_INVALID_DELTA_TIME", {"value": delta_time})
		set_error_localized("FUSE_ERROR_INVALID_DELTA_TIME", FuseError.ErrorType.RUNTIME_ERROR, {"value": delta_time})
		finished.emit()
		return
	...
	match save_to_scope:
		BaseVariable.VariableScope.LOCAL:
			VariableOperations.set_variable(context, save_to_variable, BaseVariable.VariableScope.LOCAL, delta_time)
		BaseVariable.VariableScope.SCOPE:
			if scope_source == ScopeSource.NEAREST:
				VariableOperations.set_variable(context, save_to_variable, BaseVariable.VariableScope.SCOPE, delta_time)
			else:
				...
				if scope_container == null:
					...
					finished.emit()
					return
				scope_container.set_variable(save_to_variable, delta_time)
		BaseVariable.VariableScope.GLOBAL:
			VariableOperations.set_variable(context, save_to_variable, BaseVariable.VariableScope.GLOBAL, delta_time)

	var scope_str = _get_scope_source_string()
	_log_info_localized("FUSE_LOG_DELTA_TIME_SAVED", {"time": "%.6f" % delta_time, "variable": save_to_variable, "scope": scope_str})
...
func get_description() -> String:
	var scope_name = VariableScopeUtils.enum_to_display_name(save_to_scope)
	var var_str = save_to_variable if not save_to_variable.is_empty() else FuseLocalization.translate("FUSE_VARIABLE_NOT_SPECIFIED")
	return FuseLocalization.translate_format("FUSE_INSTRUCTION_GET_DELTA_TIME_DESC_FORMAT", {"type": scope_name, "variable": var_str})
