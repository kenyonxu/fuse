...
func execute(context: ExecutionContext) -> void:
	...
	if not property_name in target:
		_log_error_localized("FUSE_ERROR_NODE_TYPE_EXPECTED", {"node": target.name, "expected": "Node with rotation property"})
		set_error_localized("FUSE_ERROR_NODE_TYPE_EXPECTED", FuseError.ErrorType.VALIDATION_ERROR, {"node": target.name, "expected": "Node with rotation property"})
		finished.emit()
		return
