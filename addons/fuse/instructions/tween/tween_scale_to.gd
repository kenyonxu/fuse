@tool
@icon("res://addons/fuse/icons/builtin/Stretch.png")
...
## Tween Scale To 指令 - 平滑缩放节点到目标大小（默认使用 EASE_OUT + TRANS_BACK）
...
var trans_type: TransitionType = TransitionType.BACK:
	set(value):
		trans_type = value
		_update_resource_name()
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_TWEEN_SCALE_TO_NAME"
	metadata.category_key = "FUSE_CATEGORY_TWEEN"
	metadata.description_key = "FUSE_INSTRUCTION_TWEEN_SCALE_TO_DESC"
	metadata.keywords = ["tween", "scale", "size", "缩放", "大小", "动画"]
	metadata.builtin_icon = "Stretch"
	return metadata
...
func _update_resource_name():
	var parts = []
	parts.append(FuseLocalization.translate("FUSE_INSTRUCTION_TWEEN_SCALE_TO_NAME"))
	var target_str := ""
	if use_variable_for_target:
...
func get_description() -> String:
	...
	# target_desc ready
	return FuseLocalization.translate_format("FUSE_INSTRUCTION_TWEEN_SCALE_TO_DESC_FORMAT", {
		"target": target_desc,
...
func execute(context: ExecutionContext) -> void:
	...
	_log_info_localized("FUSE_LOG_TWEEN_SCALE_TO", {
		"node": target.name,
		"scale": str(target_scale),
