...
func execute(context: ExecutionContext) -> void:
	...
	if not "scale" in target:
		_log_error_localized("FUSE_ERROR_NODE_TYPE_EXPECTED", {"node": target.name, "expected": "Node2D or Control"})
		set_error_localized("FUSE_ERROR_NODE_TYPE_EXPECTED", FuseError.ErrorType.VALIDATION_ERROR, {"node": target.name, "expected": "Node2D or Control"})
		finished.emit()
		return
