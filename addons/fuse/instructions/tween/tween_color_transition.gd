@tool
@icon("res://addons/fuse/icons/builtin/StyleBoxFlat.png")
...
## Tween Color Transition 指令 - 平滑改变节点颜色
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_TWEEN_COLOR_TRANSITION_NAME"
	metadata.category_key = "FUSE_CATEGORY_TWEEN"
	metadata.description_key = "FUSE_INSTRUCTION_TWEEN_COLOR_TRANSITION_DESC"
	metadata.keywords = ["tween", "color", "modulate", "颜色", "过渡", "动画"]
	metadata.builtin_icon = "StyleBoxFlat"
	return metadata
...
func _update_resource_name():
	var parts = []
	parts.append(FuseLocalization.translate("FUSE_INSTRUCTION_TWEEN_COLOR_TRANSITION_NAME"))
	var target_str := ""
	if use_variable_for_target:
...
func get_description() -> String:
	...
	# target_desc ready
	return FuseLocalization.translate_format("FUSE_INSTRUCTION_TWEEN_COLOR_TRANSITION_DESC_FORMAT", {
		"target": target_desc,
...
func execute(context: ExecutionContext) -> void:
	...
	_log_info_localized("FUSE_LOG_TWEEN_COLOR_TRANSITION", {
		"node": target.name,
		"color": str(target_color),
