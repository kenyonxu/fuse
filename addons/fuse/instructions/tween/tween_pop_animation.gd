@tool
@icon("res://addons/fuse/icons/builtin/DistractionFree.png")
extends BaseTweenInstruction
class_name TweenPopAnimation

## Tween Pop Animation 指令 - 弹簧弹出效果（缩放从 0 到目标值）
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_TWEEN_POP_ANIMATION_NAME"
	metadata.category_key = "FUSE_CATEGORY_TWEEN"
	metadata.description_key = "FUSE_INSTRUCTION_TWEEN_POP_ANIMATION_DESC"
	metadata.keywords = ["tween", "pop", "scale", "弹出", "缩放", "弹簧"]
	metadata.builtin_icon = "DistractionFree"
	return metadata
...
func _update_resource_name():
	var parts = []
	parts.append(FuseLocalization.translate("FUSE_INSTRUCTION_TWEEN_POP_ANIMATION_NAME"))
...
func get_description() -> String:
	...
	return FuseLocalization.translate_format("FUSE_INSTRUCTION_TWEEN_POP_ANIMATION_DESC_FORMAT", {
		"target": target_desc,
...
func execute(context: ExecutionContext) -> void:
	...
	_log_info_localized("FUSE_LOG_TWEEN_POP_ANIMATION", {
		"node": target.name,
		"scale": str(target_scale),
...
func validate() -> Array[String]:
	...
	if target_scale.x == 0 or target_scale.y == 0:
		errors.append(FuseLocalization.translate("FUSE_ERROR_SCALE_VALUE_NOT_ZERO"))
