...
# 元素值（按值移除时使用）- 使用 @export 使其在 Inspector 中可编辑
@export var element_value: Variant = null:
	set(value):
		element_value = value
		_update_resource_name()
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_ARRAY_REMOVE_NAME"
	metadata.category_key = "FUSE_CATEGORY_ARRAYS"
	metadata.description_key = "FUSE_INSTRUCTION_ARRAY_REMOVE_DESC"
	metadata.keywords = ["数组", "移除", "删除", "array", "remove", "delete", "元素"]
...
func _get_property_list() -> Array[Dictionary]:
	...
	properties.append({
		...
		type = TYPE_INT,
		hint = PROPERTY_HINT_ENUM,
		hint_string = "Index,Value",
		usage = PROPERTY_USAGE_DEFAULT | PROPERTY_USAGE_SCRIPT_VARIABLE
	})

	# 按索引移除时显示索引值
	if remove_mode == RemoveMode.INDEX:
		properties.append({
			name = "index_value",
			type = TYPE_INT,
			hint = PROPERTY_HINT_NONE,
	...
	# 按值移除时显示元素值配置
	if remove_mode == RemoveMode.VALUE:
		properties.append({
			name = "Element",
			type = TYPE_NIL,
			hint = PROPERTY_HINT_NONE,
	...
	# _validate_property() 会根据 use_element_from_variable 和 remove_mode 控制其可见性
...
func _update_resource_name():
	...
	match source_type:
		SourceType.VARIABLE:
			if array_variable.is_empty():
				source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_REMOVE_NO_ARRAY")
			else:
				source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_REMOVE_ARRAY", {"name": array_variable})
		SourceType.NODE_CHILDREN:
			source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_REMOVE_NODE_CHILDREN")
		SourceType.NODE_GROUP:
			if group_name.is_empty():
				source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_REMOVE_NO_GROUP")
			else:
				source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_REMOVE_GROUP", {"name": group_name})

	# 移除模式信息
	match remove_mode:
		RemoveMode.INDEX:
			mode_str = "[%d]" % index_value
		RemoveMode.VALUE:
			if use_element_from_variable:
				if element_from_variable.is_empty():
					mode_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_REMOVE_NO_ELEMENT_VAR")
				else:
					mode_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_REMOVE_FROM_VAR", {"name": element_from_variable})
			else:
	...
	resource_name = " ".join(["Array Remove", source_str, "×", mode_str])
...
func execute(context: ExecutionContext):
	...
	_log_debug_localized("FUSE_LOG_INSTRUCTION_START", {"instruction": "ArrayRemove"})
	...
	match source_type:
		SourceType.VARIABLE:
			...
			if target_array == null:
				_log_warning_localized("FUSE_WARNING_ARRAY_NOT_FOUND", {"name": array_variable})
				# 数组不存在，直接完成
	...
	if target_array.is_empty():
		_log_warning_localized("FUSE_WARNING_ARRAY_EMPTY", {})
		_on_execution_completed()
		return
	...
	match remove_mode:
		RemoveMode.INDEX:
			_remove_by_index(target_array)

		RemoveMode.VALUE:
			_remove_by_value(context, target_array)
...
## 按索引移除
func _remove_by_index(target_array: Array):
	var idx := index_value
	# 支持负索引（从末尾计数）
	if idx < 0:
	...
	if idx >= 0 and idx < target_array.size():
		var removed_element = target_array[idx]
		target_array.remove_at(idx)
		_log_info_localized("FUSE_LOG_ARRAY_REMOVED_INDEX", {"index": index_value, "element": str(removed_element)})
	else:
		_log_warning_localized("FUSE_WARNING_ARRAY_INDEX_OUT_OF_RANGE", {"index": index_value, "size": target_array.size()})

## 按值移除
func _remove_by_value(context: ExecutionContext, target_array: Array):
	var search_element: Variant
	...
	if use_element_from_variable:
		...
		match element_from_variable_scope:
			BaseVariable.VariableScope.LOCAL:
				search_element = VariableOperations.get_variable(context, element_from_variable, BaseVariable.VariableScope.LOCAL, null)
			BaseVariable.VariableScope.SCOPE:
				if element_scope_source == ScopeSource.NEAREST:
					search_element = VariableOperations.get_variable(context, element_from_variable, BaseVariable.VariableScope.SCOPE, null)
				else:
					...
					if scope_container == null:
						...
						return
					search_element = scope_container.get_variable(element_from_variable, null)
			BaseVariable.VariableScope.GLOBAL:
				search_element = VariableOperations.get_variable(context, element_from_variable, BaseVariable.VariableScope.GLOBAL, null)
	else:
		# 使用直接值
		search_element = element_value

	# 查找并移除第一个匹配项
	var idx = target_array.find(search_element)
	if idx >= 0:
		target_array.remove_at(idx)
		_log_info_localized("FUSE_LOG_ARRAY_REMOVED_VALUE", {"value": str(search_element), "index": idx})
	else:
		_log_warning_localized("FUSE_WARNING_ARRAY_VALUE_NOT_FOUND", {"value": str(search_element)})
...
func validate() -> Array[String]:
	...
	# 验证值移除模式的元素源
	if remove_mode == RemoveMode.VALUE and use_element_from_variable:
		if element_from_variable.is_empty():
...
func _validate_property(property: Dictionary) -> void:
	...
	# 移除模式相关属性
	if remove_mode == RemoveMode.INDEX:
		if property.name in ["element_value", "use_element_from_variable", "element_from_variable", "element_from_variable_scope", "element_scope_source", "element_custom_scope_id", "element_target_node_path"]:
			property.usage = PROPERTY_USAGE_NO_EDITOR
...
func get_description() -> String:
	...
	match source_type:
		SourceType.VARIABLE:
			source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_REMOVE_DESC_ARRAY", {"name": array_variable}) if not array_variable.is_empty() else FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_REMOVE_DESC_NO_ARRAY")
		SourceType.NODE_CHILDREN:
			source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_REMOVE_DESC_NODE_CHILDREN")
		SourceType.NODE_GROUP:
			source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_REMOVE_DESC_GROUP", {"name": group_name}) if not group_name.is_empty() else FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_REMOVE_DESC_NO_GROUP")

	match remove_mode:
		RemoveMode.INDEX:
			mode_str = "index %d" % index_value
		RemoveMode.VALUE:
			if use_element_from_variable:
				mode_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_REMOVE_DESC_FROM_VAR", {"name": element_from_variable}) if not element_from_variable.is_empty() else FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_REMOVE_DESC_NO_VAR")
			else:
				mode_str = str(element_value)

	return "Array Remove: %s × %s" % [source_str, mode_str]

## 重置指令状态
func reset():
	super.reset()
	_log_debug_localized("FUSE_LOG_ARRAY_REMOVE_RESET", {})
