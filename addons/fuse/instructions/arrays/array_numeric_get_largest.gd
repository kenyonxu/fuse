@tool
@icon("res://addons/fuse/icons/builtin/Max.svg")
extends BaseInstruction
class_name ArrayNumericGetLargest

## ArrayNumericGetLargest 指令
##
## 获取数值数组中的最大值并存储到目标变量。
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_ARRAY_NUMERIC_LARGEST_NAME"
	metadata.category_key = "FUSE_CATEGORY_ARRAYS"
	metadata.description_key = "FUSE_INSTRUCTION_ARRAY_NUMERIC_LARGEST_DESC"
	metadata.keywords = ["数组", "数值", "最大值", "array", "numeric", "max", "largest", "maximum"]
	metadata.builtin_icon = "Max"
	return metadata
	...
	match source_type:
		SourceType.VARIABLE:
			if array_variable.is_empty():
				source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_NUMERIC_LARGEST_NO_ARRAY")
			else:
				source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_NUMERIC_LARGEST_ARRAY", {"name": array_variable})
		SourceType.NODE_CHILDREN:
			source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_NUMERIC_LARGEST_NODE_CHILDREN")
		SourceType.NODE_GROUP:
			if group_name.is_empty():
				source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_NUMERIC_LARGEST_NO_GROUP")
			else:
				source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_NUMERIC_LARGEST_GROUP", {"name": group_name})

	# 目标信息
	if target_variable.is_empty():
		target_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_NUMERIC_LARGEST_NO_TARGET")
	else:
		target_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_NUMERIC_LARGEST_TARGET", {"name": target_variable})

	resource_name = " ".join(["Max", source_str, "→", target_str])
...
func execute(context: ExecutionContext):
	...
	_log_debug_localized("FUSE_LOG_INSTRUCTION_START", {"instruction": "ArrayNumericGetLargest"})
	...
	# 获取最大值
	var max_value: Variant = _get_numeric_max(target_array)
	...
	# 存储到目标变量
	_set_target_variable(context, max_value)

	_log_info_localized("FUSE_LOG_ARRAY_NUMERIC_LARGEST", {"value": str(max_value), "target": target_variable})

	_on_execution_completed()

## 获取数值数组的最大值
func _get_numeric_max(arr: Array) -> Variant:
	if arr.is_empty():
		return null
...
func get_description() -> String:
	...
	return "Numeric Max: %s.max() → %s" % [source_str, target_str]

## 重置指令状态
func reset():
	super.reset()
	_log_debug_localized("FUSE_LOG_ARRAY_NUMERIC_LARGEST_RESET", {})
