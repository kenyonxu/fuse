@tool
@icon("res://addons/fuse/icons/builtin/ArrowLeft.svg")
extends BaseInstruction
class_name ArrayVectorGetFurthest

## ArrayVectorGetFurthest 指令
##
## 获取向量数组中距离参考点最远的向量并存储到目标变量。
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_ARRAY_VECTOR_FURTHEST_NAME"
	metadata.category_key = "FUSE_CATEGORY_ARRAYS"
	metadata.description_key = "FUSE_INSTRUCTION_ARRAY_VECTOR_FURTHEST_DESC"
	metadata.keywords = ["数组", "向量", "最远", "距离", "array", "vector", "furthest", "distance", "Vector2", "Vector3"]
	metadata.builtin_icon = "ArrowLeft"
	return metadata
...
func _update_resource_name():
	...
	match source_type:
		SourceType.VARIABLE:
			if array_variable.is_empty():
				source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_VECTOR_FURTHEST_NO_ARRAY")
			else:
				source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_VECTOR_FURTHEST_ARRAY", {"name": array_variable})
		SourceType.NODE_CHILDREN:
			source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_VECTOR_FURTHEST_NODE_CHILDREN")
		SourceType.NODE_GROUP:
			if group_name.is_empty():
				source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_VECTOR_FURTHEST_NO_GROUP")
			else:
				source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_VECTOR_FURTHEST_GROUP", {"name": group_name})

	# 参考点信息
	if use_reference_from_variable:
		if reference_variable.is_empty():
			ref_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_VECTOR_FURTHEST_NO_REFERENCE")
		else:
	...
	if target_variable.is_empty():
		target_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_VECTOR_FURTHEST_NO_TARGET")
	else:
		target_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_VECTOR_FURTHEST_TARGET", {"name": target_variable})

	var type_str := "V2" if vector_type == VectorType.VECTOR2 else "V3"
	resource_name = " ".join(["Furthest %s" % type_str, source_str, "@", ref_str, "→", target_str])
...
func execute(context: ExecutionContext):
	...
	_log_debug_localized("FUSE_LOG_INSTRUCTION_START", {"instruction": "ArrayVectorGetFurthest"})
	...
	# 获取最远的向量
	var furthest: Variant = _get_furthest_vector(target_array, ref_point)
	...
	# 存储到目标变量
	_set_target_variable(context, furthest)

	var type_str := "Vector3" if vector_type == VectorType.VECTOR3 else "Vector2"
	_log_info_localized("FUSE_LOG_ARRAY_VECTOR_FURTHEST", {"vector": str(furthest), "type": type_str, "target": target_variable})
...
## 获取距离参考点最远的向量
func _get_furthest_vector(arr: Array, ref_point: Variant) -> Variant:
...
func get_description() -> String:
	...
	return "Get Furthest %s: %s furthest from %s → %s" % [type_str, source_str, ref_str, target_str]

## 重置指令状态
func reset():
	super.reset()
	_log_debug_localized("FUSE_LOG_ARRAY_VECTOR_FURTHEST_RESET", {})
