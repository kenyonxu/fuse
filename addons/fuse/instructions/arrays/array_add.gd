...
## 如果数组不存在则自动创建新数组。
##
## 使用 VariableOperations 统一变量访问 API（2026-02-09）
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_ARRAY_ADD_NAME"
	metadata.category_key = "FUSE_CATEGORY_ARRAYS"
	metadata.description_key = "FUSE_INSTRUCTION_ARRAY_ADD_DESC"
	metadata.keywords = ["数组", "添加", "push_back", "array", "add", "append", "元素"]
...
func _update_resource_name():
	...
	if use_element_from_variable:
		if element_from_variable.is_empty():
			element_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_ADD_NO_ELEMENT_VAR")
		else:
			element_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_ADD_FROM_VAR", {"name": element_from_variable})
	else:
	...
	resource_name = " ".join(["Array Add", source_str, "←", element_str])
...
func execute(context: ExecutionContext):
	...
	_log_debug_localized("FUSE_LOG_INSTRUCTION_START", {"instruction": "ArrayAdd"})
	...
	# 调试输出：元素值
	_log_debug("准备添加的元素值: %s (类型: %s, 来自变量: %s)" % [
		str(element),
		typeof(element),
	...
	_log_debug("数组 '%s' 添加前: 大小=%d, 内容=%s" % [array_variable, array_size_before, str(target_array)])

	target_array.push_back(element)
	...
	_log_info_localized("FUSE_LOG_ARRAY_ADDED", {"element": str(element), "array": array_variable})

	# 验证添加是否成功
	if array_size_after == array_size_before + 1:
		_log_debug("✅ 验证成功: 元素已正确添加到数组")
	else:
		_log_error("❌ 验证失败: 数组大小未增加 (期望 %d, 实际 %d)" % [array_size_before + 1, array_size_after])

	# 最终验证：重新从存储中读取数组，确认修改已持久化
	# 同时触发变量变化通知（重要：push_back 修改的是数组内容而非引用，需要手动触发信号）
	if source_type == SourceType.VARIABLE:
...
func get_description() -> String:
	...
	match source_type:
		SourceType.VARIABLE:
			source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_ADD_DESC_ARRAY", {"name": array_variable}) if not array_variable.is_empty() else FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_ADD_DESC_NO_ARRAY")
		SourceType.NODE_CHILDREN:
			source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_ADD_DESC_NODE_CHILDREN")
		SourceType.NODE_GROUP:
			source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_ADD_DESC_GROUP", {"name": group_name}) if not group_name.is_empty() else FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_ADD_DESC_NO_GROUP")

	if use_element_from_variable:
		element_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_ADD_DESC_FROM_VAR", {"name": element_from_variable}) if not element_from_variable.is_empty() else FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_ADD_DESC_NO_VAR")
	else:
		element_str = str(element_value)

	return "Array Add: %s ← %s" % [source_str, element_str]

## 重置指令状态
func reset():
	super.reset()
	_log_debug_localized("FUSE_LOG_ARRAY_ADD_RESET", {})
