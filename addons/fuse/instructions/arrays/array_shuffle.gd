@tool
@icon("res://addons/fuse/icons/builtin/Dice.svg")
extends BaseInstruction
class_name ArrayShuffle

## ArrayShuffle 指令
##
## 随机打乱数组（原地打乱，Fisher-Yates 算法）。
## 支持从变量、节点子节点或节点组获取数组。
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_ARRAY_SHUFFLE_NAME"
	metadata.category_key = "FUSE_CATEGORY_ARRAYS"
	metadata.description_key = "FUSE_INSTRUCTION_ARRAY_SHUFFLE_DESC"
	metadata.keywords = ["数组", "随机", "打乱", "洗牌", "array", "shuffle", "random", "元素"]
	metadata.builtin_icon = "Dice"
	return metadata
	...
	match source_type:
		SourceType.VARIABLE:
			if array_variable.is_empty():
				source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_SHUFFLE_NO_ARRAY")
			else:
				source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_SHUFFLE_ARRAY", {"name": array_variable})
		SourceType.NODE_CHILDREN:
			source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_SHUFFLE_NODE_CHILDREN")
		SourceType.NODE_GROUP:
			if group_name.is_empty():
				source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_SHUFFLE_NO_GROUP")
			else:
				source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_SHUFFLE_GROUP", {"name": group_name})

	resource_name = " ".join(["Array Shuffle", source_str])
...
func execute(context: ExecutionContext):
	...
	_log_debug_localized("FUSE_LOG_INSTRUCTION_START", {"instruction": "ArrayShuffle"})
	...
	var array_content_before := target_array.duplicate()
	_log_debug("数组 '%s' 打乱前: 大小=%d, 内容=%s" % [array_variable, array_size_before, str(target_array)])

	# 打乱数组（原地操作，Fisher-Yates 算法）
	target_array.shuffle()
	...
	_log_debug("════════════════════════════════════════════════════")
	_log_debug("📤 ArrayShuffle 执行结果:")
	_log_debug("  • 目标数组: '%s'" % array_variable)
	_log_debug("  • 作用域: %s" % scope_name_for_log)
	_log_debug("  • 数组大小: %d (保持不变)" % array_size_after)
	_log_debug("  • 打乱前内容: %s" % str(array_content_before))
	_log_debug("  • 打乱后内容: %s" % str(target_array))
	_log_debug("════════════════════════════════════════════════════")

	_log_info_localized("FUSE_LOG_ARRAY_SHUFFLED", {"array": array_variable})

	# 验证打乱是否成功（大小不变，元素相同）
	if array_size_after == array_size_before:
		_log_debug("验证成功: 数组大小保持不变")
		# 验证所有元素都还在
		var all_elements_present := true
		for elem in array_content_before:
			if not target_array.has(elem):
				all_elements_present = false
				break
		if all_elements_present:
			_log_debug("验证成功: 所有元素都保留在数组中")
		else:
			_log_error("验证失败: 部分元素丢失")
	else:
		_log_error("验证失败: 数组大小改变 (期望 %d, 实际 %d)" % [array_size_before, array_size_after])

	# 触发变量变化通知（重要：shuffle 修改的是数组内容而非引用，需要手动触发信号）
	# 只在 VARIABLE 模式下触发变化通知
	if source_type == SourceType.VARIABLE:
...
func _get_array_variable(context: ExecutionContext) -> Variant:
	...
	if array_value is Array:
		...
		# PackedArray 类型，转换为普通 Array 以支持 shuffle 操作
		...
		# 变量存在但不是数组，返回 null
		_log_debug("变量 '%s' 不是数组类型 (类型: %s)" % [array_variable, typeof(array_value)])
		return null
...
func get_description() -> String:
	...
	match source_type:
		SourceType.VARIABLE:
			source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_SHUFFLE_DESC_ARRAY", {"name": array_variable}) if not array_variable.is_empty() else FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_SHUFFLE_DESC_NO_ARRAY")
		SourceType.NODE_CHILDREN:
			source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_SHUFFLE_DESC_NODE_CHILDREN")
		SourceType.NODE_GROUP:
			source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_SHUFFLE_DESC_GROUP", {"name": group_name}) if not group_name.is_empty() else FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_SHUFFLE_DESC_NO_GROUP")

	return "Array Shuffle: %s" % source_str

## 重置指令状态
func reset():
	super.reset()
	_log_debug_localized("FUSE_LOG_ARRAY_SHUFFLE_RESET", {})
...
## 通知全局变量已变化（用于触发自动保存等）
## 由于 shuffle 修改的是数组内容而非引用，value_changed 信号不会自动触发
...
## 通知 SCOPE 作用域变量已变化
## 由于 shuffle 修改的是数组内容而非引用，需要调用 notify_property_list_changed
## 让远程调试器能够观测到变量变化
