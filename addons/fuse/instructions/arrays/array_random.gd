...
class_name ArrayRandom

## ArrayRandom 指令
##
## 从数组中随机获取一个元素并存储到目标变量。
## 支持从变量、节点子节点或节点组获取数组。
##
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_ARRAY_RANDOM_NAME"
	metadata.category_key = "FUSE_CATEGORY_ARRAYS"
	metadata.description_key = "FUSE_INSTRUCTION_ARRAY_RANDOM_DESC"
	metadata.keywords = ["数组", "随机", "获取", "array", "random", "pick"]
	...
	match source_type:
		SourceType.VARIABLE:
			if array_variable.is_empty():
				source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_RANDOM_NO_ARRAY")
			else:
				source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_RANDOM_ARRAY", {"name": array_variable})
		SourceType.NODE_CHILDREN:
			source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_RANDOM_NODE_CHILDREN")
		SourceType.NODE_GROUP:
			if group_name.is_empty():
				source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_RANDOM_NO_GROUP")
			else:
				source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_RANDOM_GROUP", {"name": group_name})

	# 目标信息
	if target_variable.is_empty():
		target_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_RANDOM_NO_ELEMENT_VAR")
	else:
		target_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_RANDOM_FROM_VAR", {"name": target_variable})

	resource_name = " ".join(["Array Random", source_str, "→", target_str])
...
func execute(context: ExecutionContext):
	...
	_log_debug_localized("FUSE_LOG_INSTRUCTION_START", {"instruction": "ArrayRandom"})
	...
	# 随机获取元素
	var element = target_array.pick_random()

	# 存储到目标变量
	_set_target_variable(context, element)

	_log_info_localized("FUSE_LOG_ARRAY_RANDOM", {"value": str(element), "target": target_variable})
...
func _get_array_variable(context: ExecutionContext) -> Variant:
	...
	if array_value is Array:
		...
		return array_value  # PackedArray 也支持 pick_random
...
func get_description() -> String:
	...
	return "Array Random: %s → %s" % [source_str, target_str]

## 重置指令状态
func reset():
	super.reset()
	_log_debug_localized("FUSE_LOG_ARRAY_RANDOM_RESET", {})
