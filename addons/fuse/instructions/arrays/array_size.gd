...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_ARRAY_SIZE_NAME"
	metadata.category_key = "FUSE_CATEGORY_ARRAYS"
	metadata.description_key = "FUSE_INSTRUCTION_ARRAY_SIZE_DESC"
	metadata.keywords = ["数组", "大小", "长度", "array", "size", "length", "count"]
	...
	match source_type:
		SourceType.VARIABLE:
			if array_variable.is_empty():
				source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_SIZE_NO_ARRAY")
			else:
				source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_SIZE_ARRAY", {"name": array_variable})
		SourceType.NODE_CHILDREN:
			source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_SIZE_NODE_CHILDREN")
		SourceType.NODE_GROUP:
			if group_name.is_empty():
				source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_SIZE_NO_GROUP")
			else:
				source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_SIZE_GROUP", {"name": group_name})

	# 目标信息
	if target_variable.is_empty():
		target_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_SIZE_NO_TARGET_VAR")
	else:
		target_str = target_variable

	resource_name = " ".join(["Array Size", source_str, "→", target_str])
...
func execute(context: ExecutionContext):
	...
	_log_debug_localized("FUSE_LOG_INSTRUCTION_START", {"instruction": "ArraySize"})

	# 获取数组
	var target_array: Variant = null
	...
	elif _is_packed_array(target_array):
		size = target_array.size()
	else:
		_log_error_localized("FUSE_ERROR_ARRAY_VARIABLE_NOT_ARRAY", {"name": array_variable})
		set_error_localized("FUSE_ERROR_ARRAY_VARIABLE_NOT_ARRAY", FuseError.ErrorType.RUNTIME_ERROR, {"name": array_variable})
		finished.emit()
		return
	...
	_log_info_localized("FUSE_LOG_ARRAY_SIZE", {"size": size, "target": target_variable})
...
func get_description() -> String:
	...
	return "Array Size: %s → %s" % [source_str, target_str]

## 重置指令状态
func reset():
	super.reset()
	_log_debug_localized("FUSE_LOG_ARRAY_SIZE_RESET", {})
