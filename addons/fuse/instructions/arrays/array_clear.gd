...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_ARRAY_CLEAR_NAME"
	metadata.category_key = "FUSE_CATEGORY_ARRAYS"
	metadata.description_key = "FUSE_INSTRUCTION_ARRAY_CLEAR_DESC"
	metadata.keywords = ["数组", "清空", "清除", "array", "clear", "empty", "元素"]
	...
	match source_type:
		SourceType.VARIABLE:
			if array_variable.is_empty():
				source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_CLEAR_NO_ARRAY")
			else:
				source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_CLEAR_ARRAY", {"name": array_variable})
		SourceType.NODE_CHILDREN:
			source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_CLEAR_NODE_CHILDREN")
		SourceType.NODE_GROUP:
			if group_name.is_empty():
				source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_CLEAR_NO_GROUP")
			else:
				source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_CLEAR_GROUP", {"name": group_name})

	resource_name = " ".join(["Array Clear", source_str])
...
func execute(context: ExecutionContext):
	...
	_log_debug_localized("FUSE_LOG_INSTRUCTION_START", {"instruction": "ArrayClear"})
	...
	_log_debug("数组 '%s' 清空前: 大小=%d, 内容=%s" % [array_variable, array_size_before, str(target_array)])
	...
	_log_info_localized("FUSE_LOG_ARRAY_CLEARED", {"array": array_variable})

	# 验证清空是否成功
	if array_size_after == 0:
		_log_debug("验证成功: 数组已正确清空")
	else:
		_log_error("验证失败: 数组大小不为 0 (实际 %d)" % array_size_after)

	# 触发变量变化通知（重要：clear 修改的是数组内容而非引用，需要手动触发信号）
...
func _get_array_variable(context: ExecutionContext) -> Variant:
	...
	if array_value is Array:
		...
		# PackedArray 类型，转换为普通 Array 以支持 clear 操作
...
func get_description() -> String:
	...
	match source_type:
		SourceType.VARIABLE:
			source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_CLEAR_DESC_ARRAY", {"name": array_variable}) if not array_variable.is_empty() else FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_CLEAR_DESC_NO_ARRAY")
		SourceType.NODE_CHILDREN:
			source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_CLEAR_DESC_NODE_CHILDREN")
		SourceType.NODE_GROUP:
			source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_CLEAR_DESC_GROUP", {"name": group_name}) if not group_name.is_empty() else FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_CLEAR_DESC_NO_GROUP")

	return "Array Clear: %s" % source_str

## 重置指令状态
func reset():
	super.reset()
	_log_debug_localized("FUSE_LOG_ARRAY_CLEAR_RESET", {})
...
## 通知全局变量已变化（用于触发自动保存等）
## 由于 clear 修改的是数组内容而非引用，value_changed 信号不会自动触发
...
## 通知 SCOPE 作用域变量已变化
## 由于 clear 修改的是数组内容而非引用，需要调用 notify_property_list_changed
## 让远程调试器能够观测到变量变化
