...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_ARRAY_GET_NAME"
	metadata.category_key = "FUSE_CATEGORY_ARRAYS"
	metadata.description_key = "FUSE_INSTRUCTION_ARRAY_GET_DESC"
	metadata.keywords = ["数组", "获取", "读取", "array", "get", "read", "索引"]
...
func _update_resource_name():
	...
	resource_name = " ".join(["Array Get", source_str, "[%d]" % index_value, "→", target_str])
...
func _set(property: StringName, value: Variant) -> bool:
	...
	if property == "index_value" or property == "target_variable":
		_update_resource_name()
		return false
...
func execute(context: ExecutionContext):
	...
	_log_debug_localized("FUSE_LOG_INSTRUCTION_START", {"instruction": "ArrayGet"})
	...
	# 获取元素值
	var element = target_array[idx]

	# 存储到目标变量
	_set_target_variable(context, element)

	_log_info_localized("FUSE_LOG_ARRAY_GET", {"index": index_value, "value": str(element), "target": target_variable})
...
func _get_array_variable(context: ExecutionContext) -> Variant:
	...
	if array_value is Array:
		...
		return array_value  # PackedArray 也支持索引访问
...
func get_description() -> String:
	...
	return "Array Get: %s[%d] → %s" % [source_str, index_value, target_str]

## 重置指令状态
func reset():
	super.reset()
	_log_debug_localized("FUSE_LOG_ARRAY_GET_RESET", {})
