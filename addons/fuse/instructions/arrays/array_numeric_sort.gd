...
class_name ArrayNumericSort

## ArrayNumericSort 指令
##
## 对数值数组进行排序（原地排序）。
## 支持 int 和 float 类型的数组。
## 支持升序（从小到大）和降序（从大到小）两种排序模式。
## 支持从变量、节点子节点或节点组获取数组。
##
...
# 排序模式
var sort_mode: SortMode = SortMode.ASCENDING:
	set(value):
		if sort_mode != value:
			sort_mode = value
			_update_resource_name()
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_ARRAY_NUMERIC_SORT_NAME"
	metadata.category_key = "FUSE_CATEGORY_ARRAYS"
	metadata.description_key = "FUSE_INSTRUCTION_ARRAY_NUMERIC_SORT_DESC"
	metadata.keywords = ["数组", "数值", "排序", "array", "numeric", "sort", "order", "升序", "降序"]
...
func _get_property_list() -> Array[Dictionary]:
	...
	properties.append({
		...
		type = TYPE_INT,
		hint = PROPERTY_HINT_ENUM,
		hint_string = "Ascending,Descending",
		usage = PROPERTY_USAGE_DEFAULT | PROPERTY_USAGE_SCRIPT_VARIABLE
	})
	...
	match source_type:
		SourceType.VARIABLE:
			if array_variable.is_empty():
				source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_NUMERIC_SORT_NO_ARRAY")
			else:
				source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_NUMERIC_SORT_ARRAY", {"name": array_variable})
		SourceType.NODE_CHILDREN:
			source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_NUMERIC_SORT_NODE_CHILDREN")
		SourceType.NODE_GROUP:
			if group_name.is_empty():
				source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_NUMERIC_SORT_NO_GROUP")
			else:
				source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_NUMERIC_SORT_GROUP", {"name": group_name})

	# 排序模式
	match sort_mode:
		SortMode.ASCENDING:
			mode_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_NUMERIC_SORT_ASCENDING")
		SortMode.DESCENDING:
			mode_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_NUMERIC_SORT_DESCENDING")

	resource_name = " ".join(["Sort", source_str, mode_str])
...
func execute(context: ExecutionContext):
	...
	_log_debug_localized("FUSE_LOG_INSTRUCTION_START", {"instruction": "ArrayNumericSort"})
	...
	# 验证数组元素是否为数值类型
	if not _is_numeric_array(target_array):
		_log_error_localized("FUSE_ERROR_ARRAY_NOT_NUMERIC", {})
		set_error_localized("FUSE_ERROR_ARRAY_NOT_NUMERIC", FuseError.ErrorType.RUNTIME_ERROR, {})
		finished.emit()
		return

	# 调试输出：排序前的数组状态
	var array_before := (target_array as Array).duplicate()
	_log_debug("数组排序前: 大小=%d, 内容=%s" % [target_array.size(), str(target_array)])

	# 排序数组
	_sort_numeric_array(target_array)
	...
	var mode_str := "升序" if sort_mode == SortMode.ASCENDING else "降序"
	_log_debug("════════════════════════════════════════════════════")
	_log_debug("📤 ArrayNumericSort 执行结果:")
	...
	_log_info_localized("FUSE_LOG_ARRAY_NUMERIC_SORTED", {"array": array_variable, "mode": mode_str})

	# 只在 VARIABLE 模式下触发变量变化通知
	# sort() 修改的是数组内容而非引用，需要手动触发信号
	if source_type == SourceType.VARIABLE:
...
## 检查数组是否为数值数组
func _is_numeric_array(arr: Array) -> bool:
	for item in arr:
		if not (item is int or item is float):
			_log_debug("数组包含非数值元素: %s (类型: %s)" % [str(item), typeof(item)])
			return false
	return true

## 对数值数组进行排序（升序）
func _sort_numeric_array(arr: Array) -> void:
	arr.sort()
...
func get_description() -> String:
	...
	match source_type:
		SourceType.VARIABLE:
			source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_NUMERIC_SORT_DESC_ARRAY", {"name": array_variable}) if not array_variable.is_empty() else FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_NUMERIC_SORT_DESC_NO_ARRAY")
		SourceType.NODE_CHILDREN:
			source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_NUMERIC_SORT_DESC_NODE_CHILDREN")
		SourceType.NODE_GROUP:
			source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_NUMERIC_SORT_DESC_GROUP", {"name": group_name}) if not group_name.is_empty() else FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_NUMERIC_SORT_DESC_NO_GROUP")

	return "Numeric Sort: %s (%s)" % [source_str, mode_str]

## 重置指令状态
func reset():
	super.reset()
	_log_debug_localized("FUSE_LOG_ARRAY_NUMERIC_SORT_RESET", {})
...
## 通知全局变量已变化（用于触发自动保存等）
## 由于 sort() 修改的是数组内容而非引用，value_changed 信号不会自动触发
...
## 通知 SCOPE 作用域变量已变化
## 由于 sort() 修改的是数组内容而非引用，需要调用 notify_property_list_changed
## 让远程调试器能够观测到变量变化
