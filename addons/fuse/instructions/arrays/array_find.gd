...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_ARRAY_FIND_NAME"
	metadata.category_key = "FUSE_CATEGORY_ARRAYS"
	metadata.description_key = "FUSE_INSTRUCTION_ARRAY_FIND_DESC"
	metadata.keywords = ["数组", "查找", "搜索", "索引", "array", "find", "search", "index"]
...
func _update_resource_name():
	...
	resource_name = " ".join(["Array Find", source_str, "→", target_str])
...
func execute(context: ExecutionContext):
	...
	_log_debug_localized("FUSE_LOG_INSTRUCTION_START", {"instruction": "ArrayFind"})
	...
	# 查找元素索引
	var found_index: int = target_array.find(search_value)

	# 存储到目标变量
	_set_target_variable(context, found_index)

	_log_info_localized("FUSE_LOG_ARRAY_FIND", {"value": str(search_value), "index": found_index, "target": target_variable})
...
func _get_array_variable(context: ExecutionContext) -> Variant:
	...
	if array_value is Array:
		...
		return array_value  # PackedArray 也支持 find 方法
...
func get_description() -> String:
	...
	return "Array Find: %s.find(%s) → %s" % [source_str, str(search_value), target_str]

## 重置指令状态
func reset():
	super.reset()
	_log_debug_localized("FUSE_LOG_ARRAY_FIND_RESET", {})
