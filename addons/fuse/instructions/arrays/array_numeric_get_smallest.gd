@tool
@icon("res://addons/fuse/icons/builtin/Min.svg")
extends BaseInstruction
class_name ArrayNumericGetSmallest

## ArrayNumericGetSmallest 指令
##
## 获取数值数组中的最小值并存储到目标变量。
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_ARRAY_NUMERIC_SMALLEST_NAME"
	metadata.category_key = "FUSE_CATEGORY_ARRAYS"
	metadata.description_key = "FUSE_INSTRUCTION_ARRAY_NUMERIC_SMALLEST_DESC"
	metadata.keywords = ["数组", "数值", "最小值", "array", "numeric", "min", "smallest", "minimum"]
	metadata.builtin_icon = "Min"
	return metadata
	...
	match source_type:
		SourceType.VARIABLE:
			if array_variable.is_empty():
				source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_NUMERIC_SMALLEST_NO_ARRAY")
			else:
				source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_NUMERIC_SMALLEST_ARRAY", {"name": array_variable})
		SourceType.NODE_CHILDREN:
			source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_NUMERIC_SMALLEST_NODE_CHILDREN")
		SourceType.NODE_GROUP:
			if group_name.is_empty():
				source_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_NUMERIC_SMALLEST_NO_GROUP")
			else:
				source_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_NUMERIC_SMALLEST_GROUP", {"name": group_name})

	# 目标信息
	if target_variable.is_empty():
		target_str = FuseLocalization.translate("FUSE_INSTRUCTION_ARRAY_NUMERIC_SMALLEST_NO_TARGET")
	else:
		target_str = FuseLocalization.translate_format("FUSE_INSTRUCTION_ARRAY_NUMERIC_SMALLEST_TARGET", {"name": target_variable})

	resource_name = " ".join(["Min", source_str, "→", target_str])
...
func execute(context: ExecutionContext):
	...
	_log_debug_localized("FUSE_LOG_INSTRUCTION_START", {"instruction": "ArrayNumericGetSmallest"})
	...
	# 获取最小值
	var min_value: Variant = _get_numeric_min(target_array)
	...
	# 存储到目标变量
	_set_target_variable(context, min_value)

	_log_info_localized("FUSE_LOG_ARRAY_NUMERIC_SMALLEST", {"value": str(min_value), "target": target_variable})

	_on_execution_completed()

## 获取数值数组的最小值
func _get_numeric_min(arr: Array) -> Variant:
	if arr.is_empty():
		return null
...
func _get_array_variable(context: ExecutionContext) -> Variant:
	...
	if array_value is Array:
		return array_value
	elif _is_packed_numeric_array(array_value):
		# PackedArray 需要转换为普通 Array
		var converted: Array = []
...
## 检查是否为数值类型的 PackedArray
func _is_packed_numeric_array(value: Variant) -> bool:
...
func get_description() -> String:
	...
	return "Numeric Min: %s.min() → %s" % [source_str, target_str]

## 重置指令状态
func reset():
	super.reset()
	_log_debug_localized("FUSE_LOG_ARRAY_NUMERIC_SMALLEST_RESET", {})
