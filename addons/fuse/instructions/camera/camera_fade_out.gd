...
class_name CameraFadeOut

## Camera Fade Out 指令 - 从透明渐变到全屏颜色（淡出效果）
##
## 创建一个 CanvasLayer + ColorRect 覆盖层，通过 Tween 渐变 alpha 实现淡出。
...
var easing_type: FadeEasing = FadeEasing.EASE_IN:
	set(value):
		easing_type = value
		_update_resource_name()
...
static func _get_instruction_metadata() -> InstructionMetadata:
	...
	metadata.name_key = "FUSE_INSTRUCTION_CAMERA_FADE_OUT_NAME"
	metadata.category_key = "FUSE_CATEGORY_CAMERA"
	metadata.description_key = "FUSE_INSTRUCTION_CAMERA_FADE_OUT_DESC"
	metadata.keywords = ["camera", "fade", "out", "淡出", "摄像机", "渐变", "过渡", "transition", "screen", "black"]
...
func _update_resource_name():
	var parts = []
	parts.append(FuseLocalization.translate("FUSE_INSTRUCTION_CAMERA_FADE_OUT_NAME"))
	parts.append("(%.2fs)" % duration)
	resource_name = " ".join(parts)

## 获取指令描述
func get_description() -> String:
	return FuseLocalization.translate_format("FUSE_INSTRUCTION_CAMERA_FADE_OUT_DESC_FORMAT", {
...
func execute(context: ExecutionContext) -> void:
	...
	_log_info_localized("FUSE_LOG_CAMERA_FADE_OUT", {
		"duration": str(duration),
		"color": str(color)
	})
