...
## Preset 离线校验 CLI 入口（Task 6）
## 用法: godot --headless --path . res://addons/fuse/editor/preset_ai/validate_preset.tscn \
##        -- <file-or-dir> [<file-or-dir>...] [--report <out.json>]
## 退出码：0 = 无 error；1 = 有 error finding；2 = 参数/IO 错误
## stdout 只输出逐文件 PASS/FAIL + findings 行 + summary（引擎/codec 的 warning 走 stderr）
...
func _ready() -> void:
	...
	while i < args.size():
		...
		i += 1
	if targets.is_empty():
		printerr("用法: validate_preset.tscn -- <file-or-dir> [<file-or-dir>...] [--report <out.json>]")
		get_tree().quit(2)
		return
	...
	for target in targets:
		var result: Dictionary = PresetValidator.validate_path(target)
